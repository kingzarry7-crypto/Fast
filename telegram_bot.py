...
def openai_compatible_request(messages, api_key, base_url, model):
    ...
    payload = {
        ...
        "messages": messages,
        "temperature": 0.2,
        "max_tokens": 300,  # Caps output tokens to prevent Groq 429 errors
    }
...
def analyze_market(closes, highs, lows, opens, symbol):
    ...
    if bullish_candles >= 4:
        ...
        bullish_score += 5
    elif bearish_candles > bearish_candles:
        bearish_score += 5
...
async def ask_command(update, context):
    ...
    try:
        ...
        await update.message.reply_text(
            ...
            "Our servers are experiencing high demand or rate limits. Please try asking again in a few seconds.",
            parse_mode="HTML",
...
async def signal_command(update, context):
    ...
    try:
        ...
        print("⚠️ Signal Command Error:", error)
        try:
            await status.edit_text("❌ <b>SIGNAL ERROR</b>\n\nFailed to calculate signal. Please check the market symbol.", parse_mode="HTML")
        except Exception:
            await update.message.reply_text("❌ Signal Error: Unable to fetch market data.", disable_web_page_preview=True)
...
async def photo_message_handler(update, context):
    ...
    try:
        ...
        await update.message.reply_text(
            "⚠️ <b>Vision Service Temporarily Unavailable</b>\n\n"
            "Unable to analyze image. Please try again shortly.",
            parse_mode="HTML",
            disable_web_page_preview=True
        )
