...
async def send_long_message(message, text: str, is_raw_html: bool = False):
    """
    Sends messages formatted with Telegram HTML.
    is_raw_html=True avoids double-escaping pre-formatted HTML string inputs.
    """
...
def analyze_market(closes, highs, lows, symbol="XAU/USD"):
    ...
    rr = (
        ...
        if signal != "WAIT" and stop_loss and abs(entry - stop_loss) > 0
        else 0
    )
...
def build_signal_chart(data):
    ...
    y_min = min(
        min(lows),
        sl if signal != "WAIT" and sl else min(lows)
    )

    y_max = max(
        max(highs),
        tp3 if signal != "WAIT" and tp3 else max(highs)
    )
