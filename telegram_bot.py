...
async def handle_text(update, context):
    ...
    try:
        ...
        # === PRIORITY 1: NOTIFICATION / ALERT / REMINDER INTENT ===
        # Distinguish application capabilities vs LLM limitations
        ...
        if is_notify:
            ...
            if notify_type == "price_alert":
                # Price-break alerts are NOT yet implemented as per-user alerts
                # Existing system is admin broadcast /notify <seconds> <message>
                await update.message.reply_text(
                    f"👑 <b>KING ZARRY AI - ALERT SYSTEM</b>\n\n"
                    f"🔔 Background scheduler: <b>ACTIVE</b> (every 60s)\n"
                    f"📡 Request: <code>{html.escape(text)}</code>\n\n"
                    f"⚠️ Specific price-break alert for <b>{html.escape(notify_detail)}</b> is <b>not yet implemented</b> as a per-user alert.\n\n"
                    f"✅ What IS implemented:\n"
                    f"• <code>/notify &lt;seconds&gt; &lt;message&gt;</code> - Admin creates recurring broadcast to all users\n"
                    f"• <code>/notifications</code> - List active broadcasts\n"
                    f"• <code>/cancelnotify &lt;id&gt;</code> - Cancel broadcast\n"
                    f"• News monitor automatically alerts high-impact events\n\n"
                    f"💡 For now, use:\n"
                    f"• /{notify_detail.split('/')[0].lower()} - Live {html.escape(notify_detail)} analysis\n"
                    f"• /signal {html.escape(notify_detail)} - Full MTF signal\n\n"
                    f"<i>Your alert request is noted. Per-user price alerts are on the roadmap, but the app DOES have background notifications.</i>",
                ...
                await update.message.reply_text(
                    ...
                    f"✅ What IS implemented:\n"
                    f"• Interval broadcasts: Admin <code>/notify 3600 Your message</code> every hour\n"
                    f"• News monitor with 6h deduplication\n\n"
                    f"💡 Workaround: Ask me 'Analyze BTC tomorrow morning' when you wake up, or use /plan BTC for daily plan.\n\n"
                    f"<i>KING ZARRY AI does have background processing, but specific datetime reminders need additional implementation.</i>",
                    parse_mode="HTML", disable_web_page_preview=True
                )
                return
                ...
                await update.message.reply_text(
                    ...
                    f"📝 Request: <code>{html.escape(text)}</code>\n\n"
                    f"✅ Partial support:\n"
                    f"• Admin can set: <code>/notify 86400 BTC morning briefing</code> for 24h recurring broadcast\n"
                    f"• News monitor sends automatic high-impact alerts\n\n"
                    f"⚠️ Per-user personalized daily briefings are <b>not yet fully implemented</b> as individual schedules.\n\n"
                    f"💡 For now:\n"
                    f"• Use /plan BTC - One-day trade plan\n"
                    f"• Use /signal BTC - Live MTF signal\n"
                    f"• Ask 'Analyze BTC' anytime for fresh analysis\n\n"
                    f"<i>Application HAS scheduler capability, but per-user cron is roadmap.</i>",
                    parse_mode="HTML", disable_web_page_preview=True
                )
                return
