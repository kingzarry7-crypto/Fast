...
# ============================================================
# 👑 KING ZARRY AI - COMPLETE REPLACEMENT TELEGRAM BOT
# Connected to upgraded memory.py + ai_engine.py
# ============================================================
...
# ============================================================
# CONNECT TO UPGRADED MEMORY + AI ENGINE - ACTUAL FILES
# ============================================================
...
try:
    ...
    logger.info(f"🧠 Memory initialized: {MEMORY_DB_PATH}")
...
try:
    ...
    logger.info("🤖 AIEngine initialized with memory")
except Exception as e:
    logger.error(f"❌ AIEngine init failed: {e}")
    raise
...
Advanced assistant for trading education and market analysis.
Never reveal private chain-of-thought. Answer directly. Never invent live prices.
""".strip()

# ============================================================
# STRING HELPERS - PRESERVED
# ============================================================
...
# ============================================================
# SQLITE DATABASE FOR SUBSCRIPTIONS / USERS - king_zarry.db
# ============================================================
def db_connect():
...
# ============================================================
# UPGRADED AI WRAPPERS - ACTUAL ENGINE APIS
# ============================================================
def ask_ai(prompt: str, user_id: str = "system"):
    """
    Replaces old internal implementation.
    Uses actual AIEngine.ask(user_id, prompt, image)
    Memory history loaded + saved inside engine.
    """
    if not ai_engine:
...
def analyze_image_with_ai(image_bytes: bytes, mime_type: str, prompt: str, user_id: str = "vision_user"):
    """
    Replaces old vision implementation.
    Uses actual AIEngine.ask with image=(mime_type, image_bytes)
    """
    if not ai_engine:
        raise RuntimeError("AI Engine not initialized")
...
async def create_voice_note_file(text: str) -> BytesIO:
    """
    PRIMARY voice system: ai_engine.generate_speech(text) -> BytesIO (ElevenLabs)
    Fallback to edge_tts if ElevenLabs fails.
    """
    if ai_engine:
        try:
...
# ============================================================
# MARKET DATA & INDICATORS - PRESERVED EXACTLY
# ============================================================
...
def ai_confirm_signal(data):
    ...
    try:
        summary=(f"Symbol: {data['symbol']} ({data['interval']})\nCurrent price: {data['price']:.2f}\n"
                 ...
                 "Based ONLY on data above, is this reasonably timed setup, or does it look late/over-extended/low-quality? "
                 "Reply with exactly one word first -- CONFIRM, REJECT, or UNSURE -- then a dash and reason of 12 words or fewer.")
...
# ============================================================
# ADMIN COMMANDS - PRESERVED + RESTORED
# ============================================================
async def users_command(update, context):
    ...
    for row in rows:
        try:
            ...
            expired.append(row)
    lines=["👑 <b>KING ZARRY AI USERS</b>","",f"👥 Total subscribers: <b>{len(rows)}</b>",f"🟢 Active: <b>{len(active)}</b>",f"🔴 Expired: <b>{len(expired)}</b>","","━━━━━━━━━━━━━━━━━━"]
...
async def broadcast_command(update, context):
    ...
    for uid in all_user_ids:
        try:
            ...
            failed+=1
    await status_msg.edit_text(f"📢 <b>BROADCAST COMPLETED</b>\n\n🎯 Total Users: <b>{len(all_user_ids)}</b>\n✅ Sent Successfully: <b>{success}</b>\n🚫 Blocked/Failed: <b>{failed}</b> (Blocked: {blocked})",parse_mode="HTML")
...
async def stats_command(update, context):
    ...
    await update.message.reply_text(f"👑 <b>KING ZARRY AI STATS</b>\n\n👥 Total bot users: <b>{total_users}</b>\n💎 Total subscribers: <b>{len(rows)}</b>\n"
                                    f"🟢 Active VIP: <b>{active_count}</b>\n🔴 Expired VIP: <b>{expired_count}</b>\n\n💳 Successful payments: <b>{payment_stats['payments']}</b>\n"
                                    f"⭐ Stars received: <b>{payment_stats['stars']}</b>\n\n📋 Commands:\n/users\n/subscribers\n/stats\n/broadcast <message>\n/notify\n/notifications\n/cancelnotify\n/history",parse_mode="HTML")
...
# ============================================================
# NOTIFY SYSTEM - RESTORED /notify /notifications /cancelnotify
# ============================================================
...
# ============================================================
# AI COMMANDS - USING UPGRADED ENGINE WITH MEMORY
# ============================================================
...
# ============================================================
# MESSAGE HANDLERS - UPGRADED ENGINE + MEMORY + IMAGE + VOICE
# ============================================================
async def handle_photo(update, context):
    if not await require_subscription(update):
        return
    photo=update.message.photo[-1]
    prompt=update.message.caption or "Analyze this chart or image. If it is a financial chart, identify trend, support/resistance, market pattern, and potential setups."
    await update.message.chat.send_action("typing")
    try:
...
# ============================================================
# DISCORD INTEGRATION - PRESERVED IF CONFIGURED
# ============================================================
def start_discord_if_configured():
    ...
    try:
        ...
        logger.info("💬 Discord integration configured, will start in background thread")
        return bot
    except Exception as e:
...
# ============================================================
# MAIN - SINGLE POLLING PATH - FIXES CONFLICT ERROR
# ============================================================
def main():
    ...
    print("👑 KING ZARRY AI Starting...")
    print(f"📦 Database: {DATABASE_PATH}")
    print(f"🧠 Memory DB: {MEMORY_DB_PATH}")
    print(f"🤖 AI Provider: {AI_PROVIDER}")
    print(f"🔑 XAI Key Present: {'Yes' if XAI_API_KEY else 'No (using Groq/Gemini/OpenAI fallback)'}")
    ...
    # Payments - XTR / Stars preserved
    application.add_handler(PreCheckoutQueryHandler(precheckout_handler))
    application.add_handler(MessageHandler(filters.SUCCESSFUL_PAYMENT, successful_payment_handler))

    # Content - vision and chat through upgraded engine with persistent memory
    ...
    print("👑 King Zarry AI Telegram Bot is online with upgraded memory + AI engine + ElevenLabs voice...")
    # SINGLE polling loop - fixes Conflict: terminated by other getUpdates request
    # Note: If Railway runs 2 processes with same token, conflict can still happen at infrastructure level
