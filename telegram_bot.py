...
# Reuse shared module to keep Telegram and Discord using same table/logic
try:
    ...
    # Ensure table exists in this DB too
    try:
...
async def _process_telegram_text_pipeline(update, context, text: str, is_voice_transcription: bool = False):
    ...
    price alerts -> notification intent -> market intent -> normal AI
    is_voice_transcription: if True, we already sent "I heard" and should not duplicate checks
    """
    ...
    if is_market:
        ...
        try:
            ...
            # === OPTIONAL TAVILY WEB CONTEXT for market + news queries ===
            try:
                if tavily_search and tavily_search.should_trigger_tavily(text) and tavily_search.is_tavily_configured():
                    lower_text = text.lower()
                    needs_fundamental = any(k in lower_text for k in ["why", "news", "moving", "happened", "today", "now", "reason", "regulation", "fed", "breaking", "latest"])
                    if needs_fundamental:
                        await update.message.chat.send_action("typing")
                        web_result = await asyncio.to_thread(tavily_search.search_web, text, 5, "basic", True)
                        if web_result.get("success") and web_result.get("results"):
                            ...
                            combined_prompt = (
                                f"User asked: {text}\n\n"
                                f"Technical analysis already provided:\n"
                                f"Symbol: {symbol}\n"
    ...
    await send_long_message(update.message, answer)
    # Optional voice reply if transcription or explicit request
    lower = text.lower()
...
# === VOICE NOTE HANDLER - TELEGRAM ===
MAX_VOICE_SIZE = 10 * 1024 * 1024  # 10 MB limit

async def handle_voice(update, context):
    """Handle Telegram voice and audio messages: download -> STT -> same pipeline as text"""
    ...
    if stt_engine is None:
        await update.message.reply_text("🎙️ Voice transcription is not configured. Please set GROQ_API_KEY or STT_API_KEY.")
        return
    ...
    try:
        ...
        try:
            ...
            logger.warning(f"Failed to get Telegram file: {e}")
        ...
        try:
            ...
            logger.warning(f"Failed to download voice to drive: {e}")
            await update.message.reply_text("🎙️ I couldn't download that voice message. Please try again.")
            return

        # Validate file exists and size
        if not os.path.exists(temp_path) or os.path.getsize(temp_path) < 100:
            await update.message.reply_text("🎙️ I couldn't understand that voice message. Please try again.")
            return

        await update.message.chat.send_action("typing")
        # Transcribe via STT (blocking, run in thread)
        transcription = await asyncio.to_thread(stt_engine.transcribe_file, temp_path, file_name)

        if not transcription or not transcription.strip():
            await update.message.reply_text("🎙️ I couldn't understand that voice message. Please try again or type your request.")
            return
        ...
        # Now process through SAME pipeline as text, using authenticated user ID
        await _process_telegram_text_pipeline(update, context, transcription, is_voice_transcription=True)

    except Exception as e:
        logger.error(f"Voice handler error: {e}")
        await update.message.reply_text("🎙️ I couldn't understand that voice message. Please try again.", disable_web_page_preview=True)
    finally:
        # Cleanup temp files
        try:
