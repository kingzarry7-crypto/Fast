...
def analyze_market(
    ...
    # --------------------------------------------------------
    # 👑 AUTOMATIC DIRECTION WITH BAD MARKET FILTER
    # --------------------------------------------------------
    ...
    return {
        ...
        "signal": signal,
        "trend": (
            "BULLISH" if bullish_percent > bearish_percent
            else "BEARISH" if bearish_percent > bullish_percent
            else "NEUTRAL"
        ),
        "structure": structure,
        "strength": strength,
...
def format_signal(data):
    ...
    if signal == "WAIT":
        return (
            ...
            f"💡 <i>Market is choppy or lacking clear directional momentum. "
            f"Do not force low-quality trades. Wait for a clear breakout.</i>"
        )
    ...
    return (
        ...
        f"⚠️ <i>This is algorithmic market analysis, not guaranteed financial advice. "
        f"Use appropriate position sizing and risk management.</i>"
    )
    ...
    try:
        ...
        try:
            ...
            if data["signal"] != "WAIT":
                ...
                caption = (
                    f"👑 KING ZARRY AI\n"
                    f"⚠️ WAIT / NO TRADE • {data['symbol']} • 15M\n"
                    f"Market is sideways. Hold positions."
                )
    ...
    try:
        ...
        try:
            ...
            if data["signal"] != "WAIT":
                ...
                caption = (
                    f"👑 KING ZARRY AI\n"
                    f"⚠️ WAIT / NO TRADE • {data['symbol']} • {data['interval']}\n"
                    f"Market is sideways. Hold positions."
                )
