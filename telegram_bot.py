...
def detect_fresh_cross(
    ...
    # Looks at whether the fast/slow EMA relationship flipped
    # sign within the last `lookback` candles. A fresh cross
    # means a trend may just be starting (a genuinely "on time"
    # entry); a cross that happened many candles ago means the
    # move is already underway/aging, which is exactly the kind
    # of late entry a subscriber shouldn't be chasing.
...
def detect_exhaustion(
    ...
    # Flags moves that have already run far from their EMA21 on
    # an extreme RSI reading -- the classic "it already happened,
    # don't chase it" setup that produces bad, late entries.
...
def detect_volatility_spike(
    ...
    # Compares the most recent ATR to a slightly older baseline
    # ATR. A sudden expansion usually means a news spike or a
    # choppy, whipsaw tape -- exactly the "bad market" condition
    # where the honest answer is WAIT, not a coin-flip call.
...
def analyze_market(
    ...
    fresh_cross, cross_direction = detect_fresh_cross(
        closes
    )

    is_exhausted = detect_exhaustion(
        price,
        ema21,
        current_atr,
        current_rsi
    ...
    is_volatile_spike = detect_volatility_spike(
        highs,
        lows,
        closes
    )
    ...
    # --------------------------------------------------------
    # 4b. TIMING: fresh cross vs. exhaustion
    #
    # A fresh EMA9/EMA21 cross rewards catching a move near its
    # start ("on time"). An exhausted, over-extended reading on
    # the other hand gets penalized on BOTH sides -- an
    # exhausted move is a bad entry regardless of direction, so
    # it should push toward WAIT, not toward a same-direction
    # trade that's just late.
    # --------------------------------------------------------
    ...
    # --------------------------------------------------------
    # FLAT / BAD-MARKET DETECTION
    #
    # Tuned so the engine leans toward giving a directional call
    # when there's genuine confluence, and falls back to WAIT
    # specifically in the cases that make for bad, late, or
    # coin-flip trades: sideways/conflicting scores, an
    # exhausted/over-extended move, or a volatility spike
    # (choppy, news-driven tape). A real WAIT state is kept on
    # purpose — a paid signal that always says BUY or SELL isn't
    # more useful to a subscriber, it's just less honest.
    # --------------------------------------------------------
    ...
    if is_flat_market:
        ...
        if is_exhausted:
            ...
            reasons.append(
                "Move looks exhausted/over-extended "
                "-- late entry risk"
            )
    ...
    return {
        ...
        "volatility_spike": is_volatile_spike,
        "candles": []
    }
...
# AI CROSS-CHECK
#
# The scoring engine above is a fixed, rules-based system --
# it can't actually see anything the numbers don't already
# capture. This step asks the bot's own configured AI model to
# look at the same summarized data and sanity-check the call
# as a second, independent opinion. It's a real second look,
# not a magic prediction: if no AI provider is configured, or
# the call fails, the original algorithmic result is used
# unchanged.
# ============================================================
...
def ai_confirm_signal(data):
    ...
    try:
        ...
        summary = (
            ...
            f"Algorithmic call: {data['signal']} "
            ...
            f"Looks exhausted/over-extended: {data['exhausted']}\n"
            f"Recent volatility spike: {data['volatility_spike']}\n"
            f"Reasons behind the call: "
            f"{'; '.join(data['reasons'])}\n\n"
            "Based ONLY on the data above, is this a reasonably "
            "timed setup, or does it look late, over-extended, "
            "or like a low-quality/choppy setup? "
            "Reply with exactly one word first -- CONFIRM, "
            "REJECT, or UNSURE -- then a dash and a plain-"
            "English reason of 12 words or fewer. No other text."
        )
        ...
        if verdict == "REJECT":
            ...
            data["confidence"] = "LOW"
            data["reasons"] = [
                f"AI cross-check flagged this setup: {reply}"
            ]
...
def format_signal(data):
    ...
    ai_line = (
        f"🤖 <b>AI cross-check:</b>\n"
        f"{html.escape(ai_verdict)}\n\n"
        if ai_verdict
        else ""
    )
...
# ============================================================
# ADMIN USERS / SUBSCRIBERS / BROADCAST
# ============================================================
...
async def broadcast_command(update, context):
    ...
    # ------------------------------------------------------------
    # FIX: previously this used `" ".join(context.args)`.
    # `context.args` is produced by python-telegram-bot splitting
    # the raw text on ALL whitespace (spaces, tabs AND newlines)
    # and re-joining with a single space -- that's what was
    # destroying every line break and collapsing the whole
    # message into one paragraph whenever /broadcast was used
    # with a typed-out message instead of as a reply.
    #
    # Fix: pull the raw text straight from the message and only
    # strip the leading "/broadcast" (or "/broadcast@BotName")
    # token, leaving every newline and space exactly as typed.
    # ------------------------------------------------------------
    ...
    if not is_reply:
        ...
        if not text_to_send and not has_media:
            await update.message.reply_text(
                "📢 <b>BROADCAST USAGE</b>\n\n"
                "1. Send text directly:\n"
                "<code>/broadcast Important update!</code>\n\n"
                "2. Send media with text:\n"
                "Attach a picture/video with caption <code>/broadcast Your message</code>\n\n"
                "3. Reply to any message:\n"
                "Reply to any text, photo, or video with <code>/broadcast</code>\n\n"
                "💡 Tip: options 1 and 3 preserve your exact "
    ...
    status_msg = await update.message.reply_text(
        f"🚀 Starting broadcast to <b>{len(all_user_ids)}</b> users...",
        parse_mode="HTML"
    )
    ...
    async def send_to_user(uid):
        if is_reply:
            # copy_message preserves the original message's exact
            # formatting/entities and line breaks -- this path was
            # already correct.
            ...
            await context.bot.copy_message(
                ...
                caption=escape_html(caption) if caption else None,
                parse_mode="HTML" if caption else None
            )
        else:
            await context.bot.send_message(
                chat_id=uid,
                text=escape_html(text_to_send),
                parse_mode="HTML",
                disable_web_page_preview=True
            )
    ...
    await status_msg.edit_text(
        ...
        f"🎯 Total Users: <b>{len(all_user_ids)}</b>\n"
        f"✅ Sent Successfully: <b>{success}</b>\n"
        f"🚫 Blocked/Failed: <b>{failed}</b> (Blocked: {blocked})",
        parse_mode="HTML"
    )
...
async def handle_photo(update, context):
    ...
    prompt = update.message.caption or (
        "Analyze this chart or image. If it is a financial chart, "
        "identify trend, support/resistance, market pattern, and potential setups."
    )
    ...
    try:
        ...
        await update.message.reply_text(
            "❌ <b>Vision Analysis Error</b>\n\n"
            "Unable to analyze image. Ensure vision API keys are valid.",
            parse_mode="HTML"
        )
...
async def handle_text(update, context):
    ...
    try:
        ...
        await update.message.reply_text(
            "⚠️ AI Service Temporarily Unavailable. Please try again shortly.",
            disable_web_page_preview=True
        )
...
def main():
    ...
    print("👑 King Zarry AI Telegram Bot is online and running...")
