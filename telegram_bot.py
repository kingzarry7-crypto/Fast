...
# NEW:
# • TradingView-style signal chart
# • Entry zone
# • Stop loss
# • TP1 / TP2 / TP3
# • Support / resistance
# • Market structure
# • BUY / SELL / WAIT engine
# • Automatic chart image sent with every /signal
# =========================================================
...
def analyze_market(closes, highs, lows, symbol="XAU/USD"):
    ...
    # Demand / supply zones around recent swing extremes.
    ...
    # Distance from current price to the zones.
    near_demand = current_price <= (
        demand_high + current_atr * 0.65
    )

    near_supply = current_price >= (
        supply_low - current_atr * 0.65
    )
    ...
    if bullish_score >= 5 and near_demand:
        signal = "BUY"
        direction = "🟢"
        trend = "BULLISH"
        score = bullish_score

    elif bearish_score >= 5 and near_supply:
        signal = "SELL"
        direction = "🔴"
        trend = "BEARISH"
        score = bearish_score
    ...
    if signal == "BUY":
        entry_low = max(
            demand_low,
            current_price - current_atr * 0.25
        )

        entry_high = min(
            demand_high,
            current_price + current_atr * 0.15
        )

        entry = (entry_low + entry_high) / 2

        stop_loss = demand_low - current_atr * 0.35
        ...
        # Never allow TP3 to be below TP2.
        tp3 = max(tp3, tp2)
        ...
        stop_loss = supply_high + current_atr * 0.35
        ...
        entry = current_price
        entry_zone_low = min(demand_low, supply_low)
        entry_zone_high = max(demand_high, supply_high)
    ...
    # Confidence is a signal-quality score, not a probability.
    strength = min(
        95,
        max(
            50,
    ...
    rr = (
        ...
        if signal != "WAIT" and abs(entry - stop_loss) > 0
        else 0
    )
...
def format_signal(data):
    ...
    if signal == "BUY":
        setup_text = "Price is testing a demand/support area."
    elif signal == "SELL":
        setup_text = "Price is testing a supply/resistance area."
    else:
        setup_text = (
            "No clean entry confirmation at the current price. "
            "Wait for price to reach a stronger zone."
        )
    ...
    return (
        ...
        f"🛑 Stop Loss: <code>{data['stop_loss']:,.2f}</code>\n"
        f"🎯 TP1: <code>{data['tp1']:,.2f}</code>\n"
        f"🎯 TP2: <code>{data['tp2']:,.2f}</code>\n"
        f"🎯 TP3: <code>{data['tp3']:,.2f}</code>\n\n"
...
def build_signal_chart(data):
    """
    Creates a TradingView-inspired signal chart:
    • Candlesticks
    • Demand / supply zone
    • Entry zone
    • Stop-loss zone
    ...
    y_min = min(
        min(lows),
        sl if signal != "WAIT" else min(lows)
    )

    y_max = max(
        max(highs),
        tp3 if signal != "WAIT" else max(highs)
    )
    ...
    ax.set_title(
        f"👑 KING ZARRY AI • {data['symbol']} • "
        f"{data['interval']} • {title_signal}",
        fontsize=15,
        fontweight="bold",
        loc="left",
        pad=12
...
async def start_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    welcome_text = (
        ...
        "📈 <b>New signal engine:</b> Entry zone + SL + TP1/TP2/TP3 + "
        "market structure + TradingView-style chart.\n\n"
        "👑 <b>KING ZARRY AI</b>"
    )
...
async def precheckout_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not payload.startswith("kingzarry_subscription:"):
        await query.answer(
            ok=False,
            error_message="Invalid subscription payment."
        )
        return
    ...
    if plan_key not in SUBSCRIPTION_PLANS:
        await query.answer(
            ok=False,
            error_message="This subscription plan is no longer available."
        )
        return
    ...
    if query.currency != "XTR":
        await query.answer(
            ok=False,
            error_message="Payment must be made using Telegram Stars."
        )
        return
    ...
    if query.total_amount != expected_amount:
        await query.answer(
            ok=False,
            error_message="Payment amount does not match the subscription plan."
        )
        return
...
async def successful_payment_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if plan_key not in SUBSCRIPTION_PLANS:
        await message.reply_text(
            "⚠️ Payment received, but the subscription plan could not be identified."
        )
        return
    ...
    if not recorded:
        await message.reply_text(
            ...
            "Your existing VIP subscription remains protected."
        )
        return
    ...
    await message.reply_text(
        ...
        "Use /signal XAU 1h for the advanced signal.",
        parse_mode="HTML"
    )
...
async def signal_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Default signal is now XAUUSD 1H, matching the style
    # of the chart the user provided.
    symbol, timeframe = detect_market_and_timeframe(
        raw_args or "XAUUSD 1h"
    )
    ...
    status_message = await update.message.reply_text(
        ...
        "🧠 Checking structure, EMA, RSI, support/resistance...\n"
    ...
    try:
        ...
        try:
            ...
            caption = (
                f"👑 KING ZARRY AI\n"
                f"{data['direction']} {data['signal']} • "
                f"{data['symbol']} • {data['interval']}\n"
                f"Entry: {data['entry_zone_low']:,.2f} - "
            ...
            print(
                f"⚠️ Signal chart error: {chart_error}"
            )

            await update.message.reply_text(
                "⚠️ Signal calculated successfully, "
                "but the chart image could not be generated."
            )
...
async def text_message_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if is_voice_request:
        ...
        try:
            prompt = (
                "Extract or write the exact message to be spoken "
                "based on this request. Return ONLY the exact "
                "script/words to speak with zero introductions, "
...
async def paysupport_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        ...
        "If you paid but your VIP subscription did not activate, "
        "contact the bot administrator and provide your Telegram "
        "payment information.\n\n"
...
def main():
    ...
    print("👑 =======================================")
    print(f"⭐ Monthly: {MONTHLY_STARS} XTR")
    print(f"⭐ 3 Month: {THREE_MONTH_STARS} XTR")
    print(f"⭐ Yearly: {YEARLY_STARS} XTR")
    print(f"👑 Admins: {len(ADMIN_IDS)}")
    print("📈 TradingView-style chart engine: ENABLED")
    print("🎯 Entry + SL + TP1 + TP2 + TP3: ENABLED")
    print("🧱 Support / resistance: ENABLED")
    print("🏗 Market structure: ENABLED")
    ...
    application.add_handler(
        CommandHandler("3month", three_month_command)
    )

    application.add_handler(
        CommandHandler("yearly", yearly_command)
    )
    ...
    application.add_handler(
        CommandHandler("eth", eth_command)
    )

    application.add_handler(
        CommandHandler("sol", sol_command)
    )

    application.add_handler(
        CommandHandler("xau", xau_command)
    )
    ...
    print("📈 /signal now creates the advanced chart signal.")
