...
def normalize_alert_symbol(raw: str) -> Optional[str]:
    ...
    # try to extract from string containing symbol
...
def parse_alert_request(text: str) -> Optional[Dict]:
    """
    Parse natural language or command style alert requests.
    Returns dict with symbol, target_price, condition or None
    Supports:
    - /alert XAU above 4329
    - Alert me when XAU reaches 4329
    - Notify me when gold hits 4329
    - Tell me when ETH goes above 5000
    ...
    if not symbol:
        # try from whole original
        symbol = normalize_alert_symbol(original)
    if not symbol:
        return None

    # Determine condition
    condition = "REACHES"  # default
    if re.search(r"\babove\b|\bgoes above\b|\bgo above\b|\b>\b|\bhigher than\b|\bover\b", lower):
        # need to distinguish above vs below - check explicit
        if re.search(r"\babove\b|\b>\b|\bhigher than\b|\bover\b|\bgoes above\b|\bgo above\b", lower):
            # if also contains below, need to prioritize closer to price
            # simple: if "above" in text and not "below" near price, use ABOVE
            if "below" not in lower or lower.index("above") < lower.index("below") if "above" in lower and "below" in lower else True:
                condition = "ABOVE"
    if re.search(r"\bbelow\b|\bgoes below\b|\bgo below\b|\b<\b|\blower than\b|\bunder\b|\bdrops below\b", lower):
        # if both above and below present, decide by last occurrence
        if "above" in lower and "below" in lower:
            condition = "ABOVE" if lower.rfind("above") > lower.rfind("below") else "BELOW"
        else:
            condition = "BELOW"
    if re.search(r"\breaches\b|\breach\b|\bhits\b|\bhit\b|\breach\b|\bat\b.*\b4329\b|\breaches\b", lower):
        # if explicitly says reaches/hits and no above/below, use REACHES
        if "above" not in lower and "below" not in lower:
            condition = "REACHES"
        # if pattern is "reaches 4329" without above/below, keep REACHES

    # Command style: /alert XAU above 4329
...
def get_current_price_for_alert(symbol: str) -> Optional[float]:
    """
    Reuse existing market price function. Try market.py get_price first, fallback to telegram_bot get_market_candles.
    """
    # Try market.py
    try:
    ...
    # Fallback: use telegram_bot's own TwelveData call via candles
    try:
        candles = get_market_candles(symbol, interval="1min", outputsize=10)
        if candles:
            last_close = candles[-1].get("close")
            if last_close:
                return float(last_close)
    except Exception as e:
        logger.debug(f"get_market_candles fallback failed for {symbol}: {e}")
    return None

def check_alert_triggered(current_price: float, target_price: float, condition: str, last_price: Optional[float] = None) -> bool:
    """
    Sensible crossing logic to avoid requiring exact equality.
    ABOVE: current >= target
    BELOW: current <= target
    REACHES: triggers if price crosses target in either direction or is within 0.05% tolerance
    For REACHES we use crossing detection if last_price available, otherwise simple >= / <= logic with tolerance
    """
    ...
    if condition == "REACHES":
        # If we have last price, detect crossing
        if last_price is not None:
            ...
            if (last_price < target_price <= current_price) or (last_price > target_price >= current_price):
                return True
            # Also trigger if within very close range (0.1%)
            tolerance = abs(target_price) * 0.001
            if abs(current_price - target_price) <= tolerance:
                return True
            return False
        else:
            # No last price: trigger if close enough or crossed logic using >= <= with small tolerance
            tolerance = abs(target_price) * 0.001
            return abs(current_price - target_price) <= tolerance or current_price == target_price or (current_price >= target_price and target_price > 0) and False  # default to tolerance only for REACHES without history
            # For initial check, we use tolerance + also allow >= if price already beyond target in a way that suggests it reached?
            # Safer: trigger if price is within 0.1% OR if current price is on or beyond target AND we don't have history, we consider it reached if it's within 0.5%
            # To avoid missing alerts, we trigger REACHES if abs diff <= 0.5% OR if last_price is None and current >= target and target was below recent high? Simplify: use 0.3% tolerance
    return False

# Improved REACHES logic with tolerance
def check_alert_triggered_v2(current_price: float, target_price: float, condition: str, last_price: Optional[float] = None) -> bool:
    ...
    if condition == "BELOW":
        return current_price <= target_price
    # REACHES
    # Trigger if crossed or within 0.2% tolerance
    tolerance = max(abs(target_price) * 0.002, 0.01)  # 0.2% or 1 cent minimum
    if abs(current_price - target_price) <= tolerance:
        return True
    if last_price is not None:
        if (last_price < target_price <= current_price) or (last_price > target_price >= current_price):
...
def detect_market_intent(text: str):
    """
    Intelligent market/trading intent detection for normal text messages.
    Returns: (is_market_request: bool, symbol: str, timeframe: str)
    Uses existing detect_market_and_timeframe but only triggers when intent keywords present.
    """
    if not text:
    ...
    # If text is short (<35 chars) and contains market symbol, treat as intent
    # e.g. "Analyze BTC", "BTC?", "XAU now"
    is_short_market = len(text.strip()) < 35 and has_market

    has_intent = any(kw in lower for kw in intent_keywords) or is_short_market

    # Exclude pure educational questions that mention BTC but ask about concept
    # e.g. "What is RSI" should NOT trigger BTC even if user typed BTC earlier - but our has_market check already protects
    # If text contains "what is rsi", "explain rsi", etc without market action verb + market symbol combo, we still route to AI
    # The intent list includes "what is" but we require has_market, so "Explain RSI" -> has_market False -> goes to AI (correct)
...
def detect_notification_intent(text: str):
    """
    Detect notification/alert/reminder requests.
    Returns: (is_notification: bool, type: str, details: str)
    """
    if not text:
        return False, "", ""
    lower = text.lower()
...
async def notification_job(context: ContextTypes.DEFAULT_TYPE):
    # === EXISTING ADMIN BROADCASTS - MUST CONTINUE EXACTLY AS BEFORE ===
    conn=db_connect()
    try:
        now=datetime.now(timezone.utc)
    ...
    # === NEW: PERSONAL PRICE ALERTS - SEPARATE, DOES NOT INTERFERE WITH ADMIN BROADCASTS ===
    try:
...
async def check_personal_price_alerts_job(context: ContextTypes.DEFAULT_TYPE):
    """
    Check active personal price alerts every 60s alongside admin broadcasts.
    One bad alert must not stop the scheduler.
    """
    try:
        active_alerts = get_all_active_price_alerts()
        if not active_alerts:
            return
        # Group alerts by symbol to avoid duplicate price fetches
            ...
            try:
                ...
                for alert in alerts:
                    try:
                        ...
                        # Update last checked price always
                        try:
                            conn = db_connect()
                            try:
                                conn.execute(
                        ...
                        if triggered:
                            # Send alert to owner only
                            user_id = int(alert["user_id"])
                            target = float(alert["target_price"])
                            cond = alert["condition"]
                            cond_text = {"ABOVE": f"≥ {target}", "BELOW": f"≤ {target}", "REACHES": f"reaches {target}"}.get(cond, cond)
                            ...
                            try:
                                ...
                                # Don't deactivate on send failure, retry next cycle
...
# PERSONAL PRICE ALERT COMMANDS - SEPARATE FROM ADMIN /notify
...
async def handle_text(update, context):
    ...
    try:
        ...
        for kw in ["BTC", "ETH", "SOL", "XAU", "GOLD"]:
            if kw in upper and len(text) < 40:
                try:
        ...
        # === PRIORITY 1: PERSONAL PRICE ALERTS (NEW - SEPARATE FROM ADMIN /notify) ===
        # Parse natural language alert requests and create personal alerts
        ...
        if parsed_alert:
            ...
            # Security: user_id from Telegram update, not from text
        ...
        # === PRIORITY 1b: REMINDER / BRIEFING INTENT (still not per-user timed, but honest) ===
        ...
        if is_notify:
            ...
            if notify_type == "price_alert":
                # If parse failed, ask for missing info instead of fake creation
                await update.message.reply_text(
        ...
        if is_market:
            # Route through EXISTING market engine (same as /btc, /signal)
            logger.info(f"Market intent detected in normal text: '{text}' -> {symbol}")
            ...
            try:
                ...
                try:
                    ...
                    logger.warning(f"Chart error in handle_text routing: {chart_error}")
                try:
                ...
                try:
                    ...
                    pass
                # Fall through to normal AI conversation
...
def main():
    ...
    # Personal price alerts - separate from admin /notify
