...
def analyze_market(closes, highs, lows, symbol="XAU/USD"):
    ...
    if current_rsi > 50:
        if current_rsi >= 70 and not rsi_rising:
            bearish_score += 1.5  # Overbought turning down
        elif current_rsi >= 70 and rsi_rising:
            bullish_score += 1.0  # Strong momentum continuation
        else:
            bullish_score += 1.5
    else:
        if current_rsi <= 30 and rsi_rising:
            bullish_score += 1.5  # Oversold turning up
        elif current_rsi <= 30 and not rsi_rising:
            bearish_score += 1.0  # Downward momentum
        else:
            bearish_score += 1.5

    # 4. Market Structure & Recent Highs/Lows
    if structure == "BULLISH":
        bullish_score += 2.0
    elif structure == "BEARISH":
        bearish_score += 2.0

    # 5. Support / Resistance & Pullbacks/Retests
    ...
    if dist_to_support <= current_atr * 1.2:
        bullish_score += 2.0  # Pullback to support
    if dist_to_resistance <= current_atr * 1.2:
        bearish_score += 2.0  # Retest of resistance
    ...
    recent_opens = [float(c["open"]) for c in get_market_candles(symbol, "15min", 3)[-3:]] if len(closes) >= 3 else closes[-3:]
    ...
    # Decision Matrix Thresholds (Scaled to 100)
    total_max = 11.5
    norm_bullish = min(100, int((bullish_score / total_max) * 100))
    norm_bearish = min(100, int((bearish_score / total_max) * 100))

    score_diff = abs(norm_bullish - norm_bearish)

    if norm_bullish >= 68 and norm_bullish > norm_bearish and score_diff >= 15:
        signal = "BUY"
        direction = "🟢"
        trend = "BULLISH"
        strength = norm_bullish
    elif norm_bearish >= 68 and norm_bearish > norm_bullish and score_diff >= 15:
        signal = "SELL"
        direction = "🔴"
        trend = "BEARISH"
    ...
    # Risk-Reward Target Calculation Engine
    if signal == "BUY":
        entry_low = max(support, current_price - current_atr * 0.25)
        entry_high = current_price
        entry = (entry_low + entry_high) / 2
        stop_loss = max(support - current_atr * 0.4, entry - current_atr * 1.2)
        ...
        entry_high = min(resistance, current_price + current_atr * 0.25)
        entry = (entry_low + entry_high) / 2
        stop_loss = min(resistance + current_atr * 0.4, entry + current_atr * 1.2)
...
def format_signal(data):
    ...
    if signal == "BUY":
        setup_text = "Strong bullish confluence confirmed across momentum, EMA alignment, and market structure."
        sl_str = f"{data['stop_loss']:,.2f}"
        tp1_str = f"{data['tp1']:,.2f}"
        tp2_str = f"{data['tp2']:,.2f}"
        tp3_str = f"{data['tp3']:,.2f}"
    elif signal == "SELL":
        setup_text = "Strong bearish confluence confirmed across momentum, EMA alignment, and market structure."
        sl_str = f"{data['stop_loss']:,.2f}"
        tp1_str = f"{data['tp1']:,.2f}"
        tp2_str = f"{data['tp2']:,.2f}"
        tp3_str = f"{data['tp3']:,.2f}"
        ...
        setup_text = (
            "Market conditions are currently conflicting or in consolidation. "
            "Awaiting clear momentum breakout or structural retest."
        )
        sl_str = "N/A"
        tp1_str = "N/A"
        tp2_str = "N/A"
