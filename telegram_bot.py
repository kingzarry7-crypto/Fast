...
You have full voice capabilities via an automated Text-to-Speech (TTS) voice note generator.

An advanced AI assistant for:
- General questions
- Programming
- Technology
- Trading education
- BTC, ETH, SOL, Forex, Gold/XAUUSD
- Market analysis, Image/chart analysis
- Creative writing and Business ideas
...
4. For trading, never guarantee profit; explain risk.
5. IF GENERATING SCRIPT FOR VOICE OR TTS: Output ONLY the exact raw text to be spoken. Do NOT include greetings, intro headers, or meta-commentary like "Here is your message:".
"""
...
async def text_message_handler(
    ...
    # Check for voice requests first
    voice_triggers = ["voice", "speak", "talk", "audio", "voice note"]
    is_voice_request = any(trigger in user_text.lower() for trigger in voice_triggers)

    if is_voice_request:
        await update.message.chat.send_action("record_voice")
        voice_file = None
        try:
            # Instruct AI to extract ONLY the exact transcript to speak
            prompt = (
                f"Extract or write the exact message to be spoken based on this request. "
                f"Return ONLY the exact script/words to speak with zero introductions, "
                f"formatting, or commentary:\n\n{user_text}"
            )
            spoken_text = await asyncio.to_thread(ask_ai, prompt)
            
            voice_file = await create_voice_note(spoken_text)
            with open(voice_file, "rb") as voice:
                await update.message.reply_voice(
                    voice=voice,
                    caption="👑 King Zarry AI"
