...
# Support both ELEVENLABS_MODEL_ID and legacy ELEVENLABS_MODEL env names
    ...
    if ai_engine:
        try:
            ...
            if "key=" in safe_msg.lower():
                safe_msg = "ElevenLabs failed - key redacted"
            logger.warning(f"ElevenLabs via engine failed, trying female edge_tts fallback: {safe_msg}")
    try:
        import edge_tts
        temp_dir = tempfile.mkdtemp(prefix="king_zarry_tts_")
        output_file = os.path.join(temp_dir, "voice.mp3")
        # Female fallback only - never male
        communicate = edge_tts.Communicate(text, "en-US-AriaNeural")
        logger.info("TTS provider: EdgeTTS fallback | TTS voice: en-US-AriaNeural (female)")
...
def main():
    ...
    print(f"🤖 AI Provider: {AI_PROVIDER} (xAI, Groq, Gemini, OpenAI fallback)")
