...
def openai_compatible_request(messages, api_key, base_url, model, disable_tools=True):
    ...
    # Explicitly disable tool choice to prevent "Tool choice is none" API failures
    if disable_tools:
...
def gemini_request(prompt: str, image_bytes=None, mime_type=None):
    ...
    # Use updated Gemini 2.0 endpoints to eliminate 404 NOT_FOUND errors
    models_to_try = [GEMINI_MODEL, "gemini-2.0-flash", "gemini-2.0-flash-lite", "gemini-1.5-flash-latest"]
    last_error = None
...
def analyze_image_with_ai(image_bytes: bytes, mime_type: str, prompt: str) -> str:
    ...
    if AI_PROVIDER == "GROK":
        vision_model = GROK_MODEL if "vision" in GROK_MODEL else "llama-3.2-11b-vision-preview"
...
async def text_message_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Ignore commandshandled by CommandHandler
