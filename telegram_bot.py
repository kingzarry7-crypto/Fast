...
You have full voice capabilities via an automated Text-to-Speech (TTS) voice note generator. If a user asks whether you can use voice, send a voice message, or speak, confirm that you can.
...
async def text_message_handler(
    ...
    try:
        ...
        # Triggers voice conversion if user asks for voice or speaking explicitly
        voice_triggers = ["voice", "speak", "talk", "audio", "voice note"]
        if any(trigger in user_text.lower() for trigger in voice_triggers):
            await update.message.chat.send_action("record_voice")
            voice_file = None
            try:
                voice_file = await create_voice_note(answer)
                ...
                print(f"⚠️ Voice generation failed: {v_err}")
            finally:
                if voice_file and os.path.exists(voice_file):
                    try:
                        os.remove(voice_file)
