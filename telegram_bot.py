...
async def handle_voice(update, context):
    ...
    # Check if STT is actually configured
    try:
    ...
    try:
        ...
        # Validate file exists and size - STEP B
        ...
        # Short indication - don't log full transcription for privacy unless already logged by app
        preview = transcription[:500]
        await update.message.reply_text(f"🎙️ I heard: {preview}", parse_mode=None)

        # Now process through SAME pipeline as text, using authenticated user ID - STEP E pipeline
