...
def analyze_market(closes, highs, lows, symbol="XAU/USD"):
    ...
    if signal == "BUY":
        entry_low = max(demand_low, current_price - current_atr * 0.25)
        entry_high = min(demand_high, current_price + current_atr * 0.15)
        ...
        tp1 = entry + risk * 1.5
        tp2 = entry + risk * 2.25
        tp3 = max(entry + risk * 3.0, resistance)
        tp3 = max(tp3, tp2)
        entry_zone_low = entry_low
        entry_zone_high = entry_high

    elif signal == "SELL":
        entry_low = max(supply_low, current_price - current_atr * 0.15)
        entry_high = min(supply_high, current_price + current_atr * 0.25)
        entry = (entry_low + entry_high) / 2
        ...
        tp1 = entry - risk * 1.5
        tp2 = entry - risk * 2.25
        tp3 = min(entry - risk * 3.0, support)
        tp3 = min(tp3, tp2)
        entry_zone_low = entry_low
        entry_zone_high = entry_high
    ...
    strength = min(95, max(50, 50 + score * 7))
