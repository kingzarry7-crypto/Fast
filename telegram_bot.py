...
def clean_env_str(val: str, default: str = "") -> str:
    """Strips whitespace and hidden zero-width unicode characters."""
    if not val:
        return default
    # Removes zero-width spaces, joiners, and standard whitespace
...
# Provider Keys
GROK_API_KEY = clean_env_str(os.environ.get("GROK_API_KEY"))
...
# Default Models
GROK_MODEL = clean_env_str(os.environ.get("GROK_MODEL"), "grok-beta")
...
# API Base Endpoints
GROK_BASE_URL = clean_env_str(os.environ.get("GROK_BASE_URL"), "https://api.x.ai/v1")
OPENAI_BASE_URL = clean_env_str(os.environ.get("OPENAI_BASE_URL"), "https://api.openai.com/v1")
CUSTOM_BASE_URL = clean_env_str(os.environ.get("CUSTOM_BASE_URL"))  # e.g., OpenRouter or DeepSeek
...
# =========================================================
# GEMINI ENGINE WITH MODEL FALLBACK
# =========================================================
...
def gemini_request(prompt: str, image_bytes=None, mime_type=None):
    ...
    # Standard models to attempt in order of preference
    models_to_try = [GEMINI_MODEL, "gemini-2.0-flash", "gemini-1.5-flash"]
    last_error = None

    for model_name in dict.fromkeys(models_to_try):  # Preserve order, remove duplicates
        try:
...
async def signal_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await send_long_message(update.message, format_analysis(data, "BTC SIGNAL"), parse_mode="Markdown")
    except Exception as e:
