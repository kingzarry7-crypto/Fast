...
GROQ_MODEL = clean_env_str(os.getenv("GROQ_MODEL"), "qwen/qwen3.6-27b")
...
GEMINI_MODEL = clean_env_str(os.getenv("GEMINI_MODEL"), "gemini-3.6-flash")
...
- General questions, Programming, Technology, Business, Trading education, BTC, ETH, SOL, Forex, Gold/XAUUSD, Technical analysis, Chart analysis, Image analysis, Creative tasks
Rules:
...
def save_user(user):
    ...
    try:
        connection.execute("""
            ...
            VALUES (?, ?, ?, ?, ?, ?)
            ON CONFLICT(user_id) DO UPDATE SET username=excluded.username, first_name=excluded.first_name, last_name=excluded.last_name, updated_at=excluded.updated_at
...
def update_subscription_username(user_id, username):
    ...
    connection = db_connect()
    try:
        connection.execute("UPDATE subscriptions SET username = ? WHERE user_id = ?", (username, user_id))
        connection.commit()
    finally:
        connection.close()
...
def activate_subscription(user_id, username, days, plan, payment_method, payment_id=None):
    ...
    try:
        connection.execute("""
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?)
            ON CONFLICT(user_id) DO UPDATE SET username=excluded.username, plan=excluded.plan, expires_at=excluded.expires_at, payment_method=excluded.payment_method, payment_id=excluded.payment_id
...
def get_payment_history(user_id, limit=10):
    connection = db_connect()
    try:
        rows = connection.execute("SELECT plan, payment_method, amount, currency, created_at FROM payments WHERE user_id = ? ORDER BY id DESC LIMIT ?", (user_id, limit)).fetchall()
        return rows
    finally:
        connection.close()
...
def get_all_subscribers():
    ...
    try:
        rows = connection.execute("""
            SELECT s.user_id, COALESCE(NULLIF(s.username, ''), NULLIF(u.username, ''), '') AS username,
            ...
            FROM subscriptions s LEFT JOIN users u ON u.user_id = s.user_id ORDER BY s.created_at DESC
            """).fetchall()
        return rows
    finally:
        connection.close()
...
def get_payment_stats():
    connection = db_connect()
    try:
        row = connection.execute("SELECT COUNT(*) AS payments, COALESCE(SUM(amount), 0) AS stars FROM payments").fetchone()
...
def gemini_request(prompt, image_bytes=None, mime_type=None):
    ...
    if image_bytes:
        ...
        contents = [f"{SYSTEM_PROMPT}\n\nUSER:\n{prompt}"]
    models = [GEMINI_MODEL, "gemini-3.6-flash", "gemini-2.5-flash"]
    last_error = None
    for model in dict.fromkeys(models):
        try:
            response = client.models.generate_content(model=model, contents=contents, config=types.GenerateContentConfig(temperature=0.2, max_output_tokens=300))
...
def ask_ai(prompt):
    ...
    errors = []
    providers = [("Groq", GROQ_API_KEY, GROQ_BASE_URL, GROQ_MODEL), ("xAI", XAI_API_KEY, XAI_BASE_URL, XAI_MODEL), ("OpenAI", OPENAI_API_KEY, OPENAI_BASE_URL, OPENAI_MODEL)]
...
def analyze_image_with_ai(image_bytes, mime_type, prompt):
    ...
    vision_messages = [{"role": "system", "content": SYSTEM_PROMPT}, {"role": "user", "content": [{"type": "text", "text": prompt}, {"type": "image_url", "image_url": {"url": f"data:{mime_type};base64,{encoded}"}}]}]
    errors = []
    if GROQ_API_KEY:
        try:
            return openai_compatible_request(vision_messages, GROQ_API_KEY, GROQ_BASE_URL, GROQ_MODEL)
        except Exception as error:
            errors.append(f"Groq Vision: {error}")
    if XAI_API_KEY:
        try:
...
# ============================================================
# MARKET DATA & INDICATORS (unchanged)
# ============================================================
...
def analyze_market(closes, highs, lows, opens, symbol, interval=DEFAULT_TIMEFRAME):
    ...
    if signal == "BUY":
        ...
        risk = current_atr
    return {
        "symbol": symbol, "interval": interval, "price": price, "signal": signal,
        ...
        "volatility_spike": is_volatile_spike, "candles": []
    }
...
def ai_confirm_signal(data):
    ...
    try:
        summary = (
            f"Symbol: {data['symbol']} ({data['interval']})\nCurrent price: {data['price']:.2f}\n"
            ...
            f"Recent volatility spike: {data['volatility_spike']}\nReasons behind the call: {'; '.join(data['reasons'])}\n\n"
            "Based ONLY on the data above, is this a reasonably timed setup, or does it look late, over-extended, or like a low-quality/choppy setup? "
            "Reply with exactly one word first -- CONFIRM, REJECT, or UNSURE -- then a dash and a plain-English reason of 12 words or fewer. No other text."
        )
        reply = ask_ai(summary).strip()
        if not reply:
            return data
        ...
        if verdict == "REJECT":
            data["signal"] = "WAIT"
            data["confidence"] = "LOW"
            data["reasons"] = [f"AI cross-check flagged this setup: {reply}"]
        elif verdict == "UNSURE" and data["confidence"] == "MEDIUM":
            data["confidence"] = "LOW"
    except Exception as error:
        print("⚠️ AI cross-check skipped:", error)
    return data
...
def format_signal(data):
    ...
    if signal == "WAIT":
        return (
            ...
            f"⏱ Timeframe: <b>{interval_display}</b>\n\n💰 Current price:\n<code>{data['price']:,.2f}</code>\n\n"
...
def build_signal_chart(data):
    ...
    try:
        ...
        interval_display = data["interval"].replace("min", "M").replace("h", "H").replace("day", "D").upper()
        ax.set_title(f"👑 KING ZARRY AI • {data['symbol']} • {interval_display} • {title}", fontsize=15, fontweight="bold", loc="left", pad=12)
        if times:
...
async def send_subscription_invoice(update, plan_key):
    ...
    await update.message.reply_invoice(
        ...
        payload=f"kingzarry_subscription:{plan_key}", currency="XTR",
        prices=[LabeledPrice(plan["name"], plan["stars"])], provider_token=""
    )
...
async def users_command(update, context):
    ...
    if not rows:
        await update.message.reply_text("👑 <b>KING ZARRY AI USERS</b>\n\n📭 No subscribers found yet.\n\nWhen someone successfully pays with Telegram Stars, they will appear here.", parse_mode="HTML")
        return
    now = datetime.now(timezone.utc)
    active = []
    expired = []
    for row in rows:
        try:
            ...
            expired.append(row)
    lines = [f"👑 <b>KING ZARRY AI USERS</b>", "", f"👥 Total subscribers: <b>{len(rows)}</b>", f"🟢 Active: <b>{len(active)}</b>", f"🔴 Expired: <b>{len(expired)}</b>", "", "━━━━━━━━━━━━━━━━━━"]
...
# ============================================================
# FIXED BROADCAST - WORKS WITH PICTURES, VIDEOS, DOCUMENTS
# ============================================================
def is_broadcast_update(text_or_caption: str) -> bool:
    if not text_or_caption:
        return False
    return text_or_caption.strip().lower().startswith("/broadcast")
...
async def broadcast_command(update, context):
    ...
    message = update.message
    is_reply_broadcast = bool(message.reply_to_message and (message.text or "").strip().lower().startswith("/broadcast"))

    # Case 1: Reply to a message with /broadcast
    if is_reply_broadcast or (message.reply_to_message and message.text and message.text.strip() == "/broadcast"):
        target_msg = message.reply_to_message
        all_user_ids = get_all_users()
        if not all_user_ids:
            await update.message.reply_text("📭 No users found in database.")
            return
        status_msg = await update.message.reply_text(f"🚀 Broadcasting replied message to <b>{len(all_user_ids)}</b> users...", parse_mode="HTML")
    ...
    # Case 2: Direct broadcast with or without media
    raw_text = message.text or ""
    raw_caption = message.caption or ""
    # Remove the /broadcast command prefix, keep everything else including newlines
    if message.photo or message.video or message.document or message.audio or message.voice:
        # media broadcast
        cleaned_caption = re.sub(r"^/broadcast(@\w+)?\s*", "", raw_caption, count=1, flags=re.IGNORECASE).strip("\n")
        has_media = True
        text_to_send = ""
    else:
        cleaned_caption = ""
    ...
    if not has_media and not text_to_send:
        await update.message.reply_text(
            "📢 <b>BROADCAST USAGE</b>\n\n"
            "1. Text: <code>/broadcast Hello users!</code>\n"
            "2. Photo/Video + caption: attach media with caption <code>/broadcast Your caption here</code>\n"
            "3. Reply: reply to any message (text/photo/video) with <code>/broadcast</code>\n\n"
            "Works with pictures, videos, documents.",
            parse_mode="HTML"
        )
        return
    ...
    status_msg = await update.message.reply_text(f"🚀 Broadcasting to <b>{len(all_user_ids)}</b> users...", parse_mode="HTML")
    success = failed = blocked = 0

    for uid in all_user_ids:
        try:
            if has_media:
                # This preserves photo/video/document perfectly
                ...
                await context.bot.send_message(chat_id=uid, text=text_to_send, parse_mode="HTML", disable_web_page_preview=True)
            success += 1
            await asyncio.sleep(0.04)
        except Forbidden:
            blocked += 1; failed += 1
            ...
            try:
                if has_media:
                    await context.bot.copy_message(chat_id=uid, from_chat_id=message.chat_id, message_id=message.message_id, caption=cleaned_caption if cleaned_caption else None, parse_mode="HTML")
                else:
                    await context.bot.send_message(chat_id=uid, text=text_to_send, parse_mode="HTML", disable_web_page_preview=True)
                success += 1
            except Exception:
            ...
            try:
                if has_media:
                    await context.bot.copy_message(chat_id=uid, from_chat_id=message.chat_id, message_id=message.message_id, caption=cleaned_caption if cleaned_caption else None)
                else:
    ...
    await status_msg.edit_text(
        f"📢 <b>BROADCAST COMPLETED</b>\n\n🎯 Total: <b>{len(all_user_ids)}</b>\n✅ Sent: <b>{success}</b>\n🚫 Failed: <b>{failed}</b> (Blocked: {blocked})",
        parse_mode="HTML"
    )
...
async def stats_command(update, context):
    ...
    await update.message.reply_text(
        ...
        f"👥 Total bot users: <b>{total_users}</b>\n💎 Total subscribers: <b>{len(rows)}</b>\n🟢 Active VIP: <b>{active_count}</b>\n🔴 Expired VIP: <b>{expired_count}</b>\n\n"
        f"💳 Successful payments: <b>{payment_stats['payments']}</b>\n⭐ Stars received: <b>{payment_stats['stars']}</b>\n\n"
        "📋 Commands:\n/users\n/subscribers\n/stats\n/broadcast <message>\n/history",
        parse_mode="HTML"
    )
...
async def ask_command(update, context):
    ...
    try:
        ...
        print("⚠️ ask_ai Error:", error)
        await update.message.reply_text("⚠️ <b>AI Service Temporarily Unavailable</b>\n\nOur servers are experiencing high demand or rate limits. Please try again in a few seconds.", parse_mode="HTML", disable_web_page_preview=True)
...
async def quick_symbol_command(symbol, update, context):
    ...
    try:
        ...
        data = await asyncio.to_thread(ai_confirm_signal, data)
        ...
        try:
            ...
            caption = (f"👑 KING ZARRY AI\n{sig_icon} {data['signal']} • {data['symbol']} • {data['interval']}\nEntry: {data['entry_zone_low']:,.2f} - {data['entry_zone_high']:,.2f}\nSL: {data['stop_loss']:,.2f}\nTP3: {data['tp3']:,.2f}") if data["signal"] != "WAIT" else (f"👑 KING ZARRY AI\n⚠️ WAIT / NO TRADE • {data['symbol']} • {data['interval']}")
...
# ============================================================
# FIXED MESSAGE HANDLERS - IGNORE BROADCAST
# ============================================================
async def handle_photo(update, context):
    # CRITICAL FIX: Don't process broadcast photos as AI analysis
    caption = update.message.caption or ""
    if is_broadcast_update(caption):
        return
    if not await require_subscription(update):
        return
    photo = update.message.photo[-1]
    prompt = caption or ("Analyze this chart or image. If it is a financial chart, identify trend, support/resistance, market pattern, and potential setups.")
    await update.message.chat.send_action("typing")
    try:
        file = await context.bot.get_file(photo.file_id)
        image_bytes = await file.download_as_bytearray()
        analysis = await asyncio.to_thread(analyze_image_with_ai, bytes(image_bytes), "image/jpeg", prompt)
...
async def handle_text(update, context):
    ...
    if update.message.text.startswith("/"):
        return
    # CRITICAL FIX: Ignore if admin types broadcast text without slash
    if is_broadcast_update(update.message.text):
        return
    if not await require_subscription(update):
        return
    await update.message.chat.send_action("typing")
...
def main():
    ...
    # Photo handler - now ignores broadcast
    application.add_handler(MessageHandler(filters.PHOTO, handle_photo))
    application.add_handler(MessageHandler(filters.TEXT & ~filters.COMMAND, handle_text))
    print("👑 King Zarry AI Telegram Bot is online and running... FIXED BROADCAST VERSION")
