...
# PASSWORD HASHING
#
# Python standard library scrypt.
#
# Stored format:
#
...
def _create_raw_session_token() -> str:
    """
    Raw token exists only in the browser cookie.
    Only its SHA-256 hash is stored in Neon.
    """
...
class WebMemoryAdapter:
    """
    Memory adapter for web users only.
    ...
    def get_history(
        ...
        try:
            ...
            # AI history should be chronological.
            ...
            logger.warning(
                "WebMemoryAdapter.get_history failed: %s",
                type(exc).__name__,
            )
    ...
    def add_message(
        ...
        try:
            ...
            logger.warning(
                "WebMemoryAdapter.add_message failed: %s",
                type(exc).__name__,
            )
...
def _get_user_from_session_token(
    ...
    try:
        ...
        logger.warning(
            "Could not update session last_used_at"
        )
...
async def _run_web_ai(
    ...
    web_memory = WebMemoryAdapter(
        user_id
    )

    # A separate AIEngine instance is created
    # for each authenticated web user request.
    #
    # This prevents one user's memory adapter
    # from being accidentally shared with another user.

    request_engine = AIEngine(
        memory=web_memory
    )
...
def read_root():
    ...
    return {
        "status": "KingZarry AI API is active",
        "mode": "web",
        "database": "neon",
...
async def logout(
    ...
    try:
        ...
        logger.warning(
            "Logout session revoke failed: %s",
            type(exc).__name__,
        )
...
async def chat_endpoint(
    ...
    try:
        ...
        response_text = await _run_web_ai(
            user_id,
            message,
        )
