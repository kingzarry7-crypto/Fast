...
# =========================================================
# 🔒 SAFE TOOL EXECUTION LAYER - FIX FOR RAW TOOL MARKUP LEAK
...
class AIEngine:
    ...
    def _call_providers(self, prompt_text: str, history, image, persistent_ctx: str):
        ...
        for provider in finite_providers:
            try:
                ...
                if provider == "openrouter" and OPENROUTER_API_KEY:
                    resp = self._openrouter(prompt_text, history, image, persistent_ctx)
                elif provider == "groq" and GROQ_API_KEY:
                    resp = self._groq(prompt_text, history, image, persistent_ctx)
                elif provider == "gemini" and GEMINI_API_KEY:
                    resp = self._gemini(prompt_text, history, image, persistent_ctx)
    ...
    def ask(self, user_id: str, prompt: str, image = None) -> str:
        ...
        first_response = self._call_providers(prompt_for_providers, history, image, persistent_ctx)
        ...
        if _detect_forbidden_tool_attempts(first_response):
            ...
            if not sanitized or len(sanitized) < 10:
                return "Sorry, I couldn't retrieve that information due to a security policy. Please try rephrasing, e.g., 'Show my alerts'."
        ...
        if tool_requests:
            ...
            if not tool_result.get("success"):
                return "Sorry, I couldn't retrieve your alert status right now. Please try again."
            ...
            if total == 0:
                ...
                for a in alerts[:10]:
                    status = "✅ Active" if a["active"] else "⏸️ Inactive"
                    if a["triggered"]:
                        status += " 🎯 Triggered"
