...
- ✅ Telegram commands: /btc /eth /sol /xau /signal /plan /news /events /ask /tts /buy /status /alert /alerts /cancelalert + voice notes (Telegram voice/audio handled via filters.VOICE | filters.AUDIO -> stt_engine.transcribe_file -> _process_telegram_text_pipeline) + live web search via Tavily for latest news queries
- ✅ Discord commands: /btc /eth /sol /xau /gold /signal /crypto /plan /news /events /ask /tts /voice /alert /alerts /cancelalert + natural language market intent and alert detection in on_message + audio attachments (audio/* MIME or .ogg/.mp3/.m4a etc -> stt_engine.transcribe_bytes -> same pipeline as text) + live web search via Tavily

YOU MUST DISTINGUISH:
1. What you as a pure LLM can do alone: limited context window, no native scheduling
2. What the KING ZARRY AI APPLICATION can do via Python tools: persistent SQLite memory, background scheduler (Telegram JobQueue + Discord tasks loop), admin broadcasts (Telegram), personal price alerts (shared table both platforms), market engine, news engine, charts, TTS, vision, STT voice notes, Tavily live web search, etc.
...
PERSONAL PRICE ALERTS - HOW IT WORKS (NEW):
...
class AIEngine:
    def __init__(self, memory=None):
        ...
        try:
            ...
            if _tavily.is_tavily_configured():
                ...
                logger.info("ℹ️ Tavily not configured - TAVILY_API_KEY missing, continuing without web search")
        except Exception as e:
            self._tavily_module = None
            logger.info(f"ℹ️ Tavily module not available: {_redact_secrets(str(e))} - continuing without web search")

    def _get_provider_order(self) -> List[str]:
        # FINAL REQUIRED FALLBACK: OpenRouter → Groq → Gemini
        # No direct xAI, no direct OpenAI. Grok via OpenRouter if needed.
        return ["openrouter", "groq", "gemini"]

    def _should_use_tavily(self, prompt: str) -> bool:
        """Credit-conserving check: does this prompt need live web search?"""
        if not self._tavily_module:
            return False
        try:
            # Check config first to conserve credits
    ...
    def _get_tavily_context(self, prompt: str, max_results: int = 5, search_depth: str = "basic") -> Tuple[str, List[Dict[str, str]]]:
        """
        Fetch Tavily web context for prompt, return (formatted_context, sources)
        Never raises, never exposes key
        """
    ...
    def tavily_search(self, query: str, max_results: int = 5, search_depth: str = "basic") -> Dict[str, Any]:
        """Public method for direct Tavily search - used by market routing if needed"""
        if not self._tavily_module:
    ...
    def _load_persistent_context(self, user_id: str) -> str:
        """Load trading preferences + user facts + recent memory summary for injection"""
        if not self.memory:
            return ""
        parts = []
        try:
    ...
    def ask(self, user_id: str, prompt: str, image: Optional[Tuple[str, bytes]] = None) -> str:
        ...
        # === TAVILY INTEGRATION - fetch live web context if needed (credit conserving) ===
        tavily_context = ""
        tavily_sources: List[Dict[str, str]] = []
        try:
            if original_prompt and self._should_use_tavily(original_prompt):
                ...
                if ctx:
                    ...
                    prompt_for_providers = (
                        ...
                        f"{tavily_context}\n"
                        f"--- END WEB CONTEXT ---\n\n"
                        f"INSTRUCTIONS FOR WEB CONTEXT:\n"
                        f"- Use the live web search results above to answer accurately with current information\n"
                        f"- Do NOT invent prices, news, or events not in the web context\n"
                        f"- When you use web information, mention sources by title and URL when appropriate\n"
                        f"- If web context is empty or irrelevant, answer from your own knowledge but note that live data was unavailable\n"
                        f"- Combine technical reasoning with current fundamental context when relevant"
                    )
                    logger.info(f"Tavily context added for query: '{original_prompt[:60]}' -> {len(srcs)} sources")
        except Exception as e:
            logger.warning(f"Tavily integration failed (non-fatal, continuing): {_redact_secrets(str(e))}")
            tavily_context = ""
            tavily_sources = []
        ...
        for provider in finite_providers:
            try:
                ...
                if provider == "openrouter" and OPENROUTER_API_KEY:
                    resp = self._openrouter(prompt_for_providers, history, image, persistent_ctx)
                elif provider == "groq" and GROQ_API_KEY:
                    resp = self._groq(prompt_for_providers, history, image, persistent_ctx)
                elif provider == "gemini" and GEMINI_API_KEY:
                    resp = self._gemini(prompt_for_providers, history, image, persistent_ctx)
                ...
                if resp:
                    ...
                    if cleaned:
                        self._save_memory(user_id, original_prompt, cleaned)
    ...
    def _build_openai_messages(self, prompt: str, history: List[dict], image: Optional[Tuple[str, bytes]], persistent_ctx: str = "", include_system: bool = True) -> List[dict]:
        messages = []
        if include_system:
        ...
        # Inject persistent context as separate system message if needed for models that respect second system
        if persistent_ctx and not include_system:
            ...
            # Also add as assistant context for better recall
            # Keep history limited but ensure persistent context is visible
            pass
        for h in history[-10:]:
    ...
    def _xai(self, prompt: str, history: List[dict], image: Optional[Tuple[str, bytes]], persistent_ctx: str = "") -> Optional[str]:
        if not XAI_API_KEY:
            return None
        model = XAI_MODEL
        messages = self._build_openai_messages(prompt, history, image, persistent_ctx)
    ...
    def _groq(self, prompt: str, history: List[dict], image: Optional[Tuple[str, bytes]], persistent_ctx: str = "") -> Optional[str]:
        if not GROQ_API_KEY:
            return None
    ...
    def _openrouter(self, prompt: str, history: List[dict], image: Optional[Tuple[str, bytes]], persistent_ctx: str = "") -> Optional[str]:
        # PRIMARY provider: OpenRouter (openrouter/free)
    ...
    def _openai(self, prompt: str, history: List[dict], image: Optional[Tuple[str, bytes]], persistent_ctx: str = "") -> Optional[str]:
        # Backward compatibility alias: direct OpenAI is removed, now routes to OpenRouter
        # This preserves existing calls without using api.openai.com
        return self._openrouter(prompt, history, image, persistent_ctx)



    def _gemini(self, prompt: str, history: List[dict], image: Optional[Tuple[str, bytes]], persistent_ctx: str = "") -> Optional[str]:
