...
- ✅ Background notification scheduler: JobQueue runs notification_job every 60 seconds (Railway logs: "🔔 Notification job scheduled every 60s") - handles BOTH admin broadcasts AND personal price alerts
- ✅ Admin Broadcast System (SEPARATE): /notify <seconds> <message> creates recurring broadcast to all users (admin only), /notifications lists, /cancelnotify cancels - existing system preserved
- ✅ Personal Price Alerts (NEW, SEPARATE): price_alerts table in king_zarry.db, per-user isolated, conditions ABOVE/BELOW/REACHES, one-shot, checked every 60s in same scheduler but separate function check_personal_price_alerts_job(), commands /alert, /alerts, /cancelalert - DOES NOT interfere with admin broadcasts
...
- ✅ Market engine: Multi-timeframe analysis 4H→1H→15M→5M with primary 15M execution, TP/SL, late-entry and exhaustion detection - functions analyze_multi_timeframe(), format_signal_mtf(), build_signal_chart(), get_price() for alerts
- ✅ News engine: news.py economic calendar + headlines with risk levels LOW/MEDIUM/HIGH/EXTREME
- ✅ Chart generation: Matplotlib candles with EMA, support/resistance, entry zones
- ✅ Vision: Chart/image analysis via meta-llama/llama-4-scout
- ✅ TTS: ElevenLabs eleven_v3 (Bella) + Edge TTS fallback
- ✅ Telegram Stars subscriptions: monthly/3month/yearly, payments, admin, broadcast
...
2. What the KING ZARRY AI APPLICATION can do via Python tools: persistent SQLite memory, background scheduler, admin broadcasts, personal price alerts, market engine, news engine, charts, TTS, vision, etc.
3. Admin broadcasts vs Personal alerts are SEPARATE systems - do NOT confuse them
...
- Memory flow: User message → Telegram handler → memory.get_history() + get_trading_preferences() + get_facts() → AIEngine → AI response → memory.add_message() saves to SQLite
        ...
        for provider in finite_providers:
            try:
                ...
                if provider == "openrouter" and OPENROUTER_API_KEY:
                    resp = self._openrouter(prompt, history, image, persistent_ctx)
                elif provider == "groq" and GROQ_API_KEY:
                    resp = self._groq(prompt, history, image, persistent_ctx)
                elif provider == "gemini" and GEMINI_API_KEY:
                    resp = self._gemini(prompt, history, image, persistent_ctx)
                elif provider == "openai" and OPENROUTER_API_KEY:
                    # Legacy alias - still routes to OpenRouter, no direct api.openai.com
                    resp = self._openrouter(prompt, history, image, persistent_ctx)
                else:
                    continue
                if resp:
