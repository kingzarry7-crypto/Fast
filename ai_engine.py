...
class AIEngine:
    ...
    def ask(self, user_id: str, prompt: str, image=None) -> str:
        ...
        # Groq is now enabled for images via qwen/qwen3.6-27b
        if AI_PROVIDER == "GROQ":
    ...
    # =====================================================
    # GROQ PROVIDER (UPDATED FOR QWEN 3.6 MULTIMODAL VISION)
    # =====================================================
    def _groq(self, messages, image=None):
        if not GROQ_API_KEY:
            raise RuntimeError("GROQ_API_KEY is not configured.")

        # Updated check: Matches vision, qwen, or llama-4 models
    ...
    def _clean_response(self, text: str) -> str:
        ...
        # 2. Remove unclosed/truncated <think> blocks if response cuts off mid-thought
        text = re.sub(r"<think>.*$", "", text, flags=re.DOTALL)
        # 3. Strip legacy model control/special tokens
        text = re.sub(r"<\|.*?\|>", "", text)
        return text.strip()
