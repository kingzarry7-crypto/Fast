...
def analyze_market(symbol, timeframe="15m"):
    ...
    if support is None or resistance is None:
        raise RuntimeError(
            "Unable to calculate support/resistance."
        )
    ...
    if 50 <= rsi < 70:
        ...
        # Strong bullish momentum can remain overbought.
        bullish_score += 8
        reasons.append("Strong bullish RSI momentum")

    elif rsi <= 30:
        # Oversold does NOT automatically mean BUY.
        # Strong bearish momentum can remain oversold.
        bearish_score += 8
    ...
    # Strong enough directional setup
    if (
        bullish_score >= 60
    ...
    if signal == "BUY":
        ...
        # Entry is based around current price and ATR.
        entry_low = max(
            support,
            price - (atr * 0.35)
        )
        ...
        # Fibonacci-style risk multiples
        tp1 = price + (risk * 1.0)
        tp2 = price + (risk * 2.0)
        tp3 = price + (risk * 3.0)

        # Avoid placing TP directly inside resistance
        if resistance > price:
    ...
    # =====================================================
        ...
        # Stop above resistance / ATR protection
        atr_stop = price + (atr * 1.2)
        ...
        # Avoid placing TP directly inside support
        if support < price:
    ...
    # =====================================================
        ...
        # Give the bot a useful possible entry zone
        # instead of returning meaningless N/A values.
    ...
    if signal == "BUY":
        ...
        setup = (
            "Bullish confirmation detected from "
            "trend, momentum and technical structure."
        )

    elif signal == "SELL":

        setup = (
            "Bearish confirmation detected from "
            "trend, momentum and technical structure."
        )
        ...
        setup = (
            "Bullish bias detected, but entry confirmation "
        ...
        setup = (
            "Bearish bias detected, but entry confirmation "
            "is not strong enough yet."
        )
    ...
    return {
        ...
        "entry_zone": (
            f"{round_price(entry_low)} - "
            f"{round_price(entry_high)}"
