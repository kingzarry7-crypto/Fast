...
def get_user_alert_status(user_id) -> Dict:
    """
    Safe application-level function: returns structured alert status for authenticated user
    Uses parameterized SQL only, no raw SQL from model
    """
    try:
        # Normalize user_id to int or string
        conn = db_connect()
        try:
            ensure_price_alerts_table(conn)
            # Parameterized queries - user_id from authenticated context only
            total_row = conn.execute(
                "SELECT COUNT(*) as cnt FROM price_alerts WHERE user_id=?", (user_id,)
            ).fetchone()
            total = total_row["cnt"] if total_row else 0

            active_row = conn.execute(
                "SELECT COUNT(*) as cnt FROM price_alerts WHERE user_id=? AND active=1", (user_id,)
            ).fetchone()
            active = active_row["cnt"] if active_row else 0

            triggered_row = conn.execute(
                "SELECT COUNT(*) as cnt FROM price_alerts WHERE user_id=? AND triggered=1", (user_id,)
            ).fetchone()
            triggered = triggered_row["cnt"] if triggered_row else 0

            alerts_rows = conn.execute(
                "SELECT id, symbol, target_price, condition, active, triggered, created_at, triggered_at FROM price_alerts WHERE user_id=? ORDER BY created_at DESC LIMIT 20",
                (user_id,)
            ).fetchall()
        ...
        logger.error(f"get_user_alert_status failed for user {str(user_id)[:3]}***: {e}")
        return {
            "success": False,
            "error": "db_error",
            "total_alerts": 0,
