...
# === DISCORD MARKET INTENT ROUTING (same as Telegram) ===
...
def detect_news_intent_discord(text: str):
    ...
    if any(k in lower for k in news_keywords):
        ...
        # If mentions market + news, treat as news intent
        if has_market:
            return True
        # Pure news requests also
...
class KingZarryAI(discord.Client):
    ...
    async def on_message(self, message: discord.Message):
        ...
        for a in message.attachments:
            ...
            # Exclude images already counted
            if is_audio and not ct.startswith("image/"):
                audio_attachments.append(a)
        # If no text, no images, no audio -> ignore
        ...
        if audio_attachments and not content:
            ...
            # If both audio and text, prioritize text but still note audio
            # If text is caption like "transcribe this", try to transcribe anyway and append
            try:
                if stt_engine:
                    async with message.channel.typing():
                        audio_att = audio_attachments[0]
        ...
        # === PRIORITY 1: PERSONAL PRICE ALERTS (natural language) ===
        # Detect before market and before AI - reuse shared parser
        try:
            parsed = parse_alert_request_discord(content)
            if parsed and not images:
                # Validate and create using authenticated Discord user ID
                ...
                if create_fn is None:
                    # fallback to telegram_bot import if available
                    try:
        ...
        # === PRIORITY 2: MARKET INTENT ROUTING (same as Telegram) ===
        try:
            is_market, symbol, timeframe = detect_market_intent_discord(content)
            if is_market and not images:
                # Route through existing market engine - reuse same as slash commands
                async with message.channel.typing():
                    try:
        ...
        # === PRIORITY 3: NORMAL AI CONVERSATION (shared AIEngine OpenRouter→Groq→Gemini) ===
        try:
            async with message.channel.typing():
                image_tuple = None
                if images:
            ...
            if wants_voice:
                try:
                    ...
                    # For voice input, don't fail completely - still send text
...
# === BACKGROUND PRICE ALERT MONITORING FOR DISCORD (shared table, separate notifier) ===
# Reuses same price_alerts table as Telegram, but sends via Discord DM
# Does NOT interfere with Telegram's notification_job - runs as Discord task
...
# Start loop when bot is ready - hook into on_ready via extra task
...
# === DISCORD PERSONAL PRICE ALERTS - SLASH COMMANDS (shared table with Telegram) ===
...
async def alert_slash(interaction: discord.Interaction, symbol: str, condition: str, price: float):
    ...
    await interaction.response.defer(ephemeral=True)
    try:
        # Validate using authenticated Discord ID
