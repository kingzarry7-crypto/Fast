...
# Voice-aware system prompt to prevent hallucination
SYSTEM_VOICE_PROMPT = (
    "You are King Zarry AI, a multi-platform AI assistant powered by Groq, Gemini, and OpenAI. "
    "You have full voice generation capabilities powered by ElevenLabs. "
    "When asked if you can speak, generate audio, or use voice, confirm enthusiastically!"
)
...
def generate_elevenlabs_voice(text: str) -> io.BytesIO:
    """Generates audio bytes synchronously from ElevenLabs API."""
...
class KingZarryAI(discord.Client):
    ...
    async def on_message(self, message: discord.Message):
        ...
        try:
            ...
            # Check if the user is asking for voice output
            voice_triggers = ["use voice", "speak", "send audio", "voice message", "say this", "can you speak"]
            wants_voice = any(trigger in content.lower() for trigger in voice_triggers)

            if wants_voice and eleven_client:
                try:
                    # Generate voice audio off-thread
                    ...
                    # Reply with text AND audio attachment
                    await message.reply(content=answer, file=discord_file, mention_author=False)
                    return
                except Exception as voice_err:
                    print(f"⚠️ Voice generation error: {voice_err}")

            # Standard text chunks response
