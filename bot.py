...
print("🎬 Hugging Face Token:", "FOUND" if HF_TOKEN else "MISSING")
...
def generate_text_video(prompt: str) -> bytes:
    ...
    api_url = f"https://router.huggingface.co/hf-inference/models/{TEXT_TO_VIDEO_MODEL}"
    headers = {
...
def generate_image_video(image_bytes: bytes, prompt: str) -> bytes:
    ...
    api_url = f"https://router.huggingface.co/hf-inference/models/{IMAGE_TO_VIDEO_MODEL}"
    headers = {
        "Authorization": f"Bearer {HF_TOKEN}",
        "Content-Type": "application/json"
    }
