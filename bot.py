...
# FIX: Refresh market engine key after dotenv - reuses same Railway var as Telegram
try:
    _fresh = os.getenv("TWELVE_DATA_API_KEY")
    if _fresh:
        _fresh = re.sub(r"[\u200b\u200c\u200d\u2060\ufeff]", "", _fresh).strip()
        if _fresh:
            market_engine.TWELVE_DATA_API_KEY = _fresh
            try:
                import config
                config.TWELVE_DATA_API_KEY = _fresh
            except Exception:
                pass
except Exception:
    pass
...
# FIX 4: build_discord_signal_embed with DATA UNAVAILABLE guard
def build_discord_signal_embed(market_data: Dict[str, Any], news_data: Optional[Dict[str, Any]] = None) -> Embed:
    # Prevent fake $0.00 signals
    symbol_check = market_data.get("symbol", "UNKNOWN")
    raw_price_check = market_data.get("price") or market_data.get("current_price") or market_data.get("original_price")
    reason_check = str(
        market_data.get("reason")
        or market_data.get("trigger_condition")
        or market_data.get("error")
        or market_data.get("fresh_error")
        or ""
    )
    is_unavailable = (
        raw_price_check is None
        or safe_float(raw_price_check) is None
        or "DATA UNAVAILABLE" in reason_check.upper()
        or "TWELVE_DATA_API_KEY" in reason_check.upper()
        or "FAILED TO FETCH" in reason_check.upper()
    )
    if is_unavailable:
        embed = Embed(
            title=f"👑 KING ZARRY AI • {symbol_check} SIGNAL • DATA UNAVAILABLE",
            description=f"❌ **Market data unavailable for {symbol_check}**\n\n{reason_check[:1000] or 'No price returned from Twelve Data'}",
            color=discord.Color.dark_red(),
        )
        embed.add_field(
            name="🔧 Diagnosis",
            value=f"Twelve key present: {bool(getattr(market_engine, 'TWELVE_DATA_API_KEY', None) or os.getenv('TWELVE_DATA_API_KEY'))} | Railway env is used like Telegram",
            inline=False,
        )
        embed.set_footer(text="No BUY/SELL generated — real data required")
        return embed
    ...
    price_text = f"💰 Price: {fmt_price(price)}\n🎯 Entry: {fmt_price(entry_low)} - {fmt_price(entry_high)}\n🛑 SL: {fmt_price(stop_loss)}"
    embed.add_field(name="💵 PRICE", value=price_text, inline=False)
    tp_text = f"TP1: {fmt_price(tp1)}\nTP2: {fmt_price(tp2)}\nTP3: {fmt_price(tp3)}\n⚖️ RR: 1:{market_data.get('rr', 3.5)}"
    embed.add_field(name="🎯 TARGETS", value=tp_text, inline=True)
    sr_text = f"Support: {fmt_price(support)} (nearest {fmt_price(nearest_support)})\nResistance: {fmt_price(resistance)} (nearest {fmt_price(nearest_resistance)})\nStructure: {structure}"
    embed.add_field(name="🏗 LEVELS", value=sr_text, inline=True)
    ind_text = f"RSI: {f'{rsi:.1f}' if rsi is not None else 'N/A'}\nEMA9: {fmt_price(ema9)}\nEMA21: {fmt_price(ema21)}\nEMA50: {fmt_price(ema50)}\nATR: {f'{atr:,.4f}' if atr is not None else 'N/A'}"
...
def build_discord_signal_chart(symbol: str, timeframe: str, market_data: Dict[str, Any]):
    try:
        ...
        for c in chart_candles:
            try:
                opens.append(float(c.get("open", 0))); highs.append(float(c.get("high", 0))); lows.append(float(c.get("low", 0))); closes.append(float(c.get("close", 0)))
                dt_str = c.get("datetime", "") or c.get("time", "")
                try:
                    dt = datetime.fromisoformat(dt_str.replace("Z","").split(".")[0]); times.append(dt)
                except: times.append(None)
            except: continue
        if len(closes) < 10: return None
        x = list(range(len(closes)))
        def ema(values, period):
            if len(values) < period: return [float("nan")]*len(values)
            result = []; mult = 2/(period+1); sma = sum(values[:period])/period
            for i in range(len(values)):
                if i < period-1: result.append(float("nan"))
                elif i == period-1: result.append(sma)
                else:
                    sma = ((values[i] - sma)*mult) + sma; result.append(sma)
            return result
        ema21_vals = ema(closes, 21); ema50_vals = ema(closes, 50)
        ...
        try:
            ...
            for i in range(len(chart_candles)):
                ...
                bottom = min(o, c_); height = max(abs(c_ - o), 0.000001)
            ...
            support = safe_float(market_data.get("support")); resistance = safe_float(market_data.get("resistance"))
            entry_low = safe_float(market_data.get("entry_low")); entry_high = safe_float(market_data.get("entry_high"))
            sl = safe_float(market_data.get("stop_loss")); tp1 = safe_float(market_data.get("tp1")); tp2 = safe_float(market_data.get("tp2")); tp3 = safe_float(market_data.get("tp3")); atr = safe_float(market_data.get("atr"))
            if signal in ["BUY", "SELL"] and entry_low and entry_high and sl and tp3:
                x0 = max(0, len(x) - 18); box_width = 18
                try:
                    if signal == "BUY":
            ...
            for level, label in levels:
                ...
                try: ax.text(len(x)+0.8, level, f"{label} {level:,.2f}", fontsize=8, fontweight="bold")
                except: pass
            if current:
                ax.axhline(current, linewidth=1, alpha=0.5)
                try: ax.text(len(x)-1, current, f" {current:,.2f}", fontsize=9, fontweight="bold")
                except: pass
            title_sig = "🟢 BUY" if signal == "BUY" else "🔴 SELL" if signal == "SELL" else "⚠️ WAIT"
            mtf_bias = market_data.get("mtf_bias", ""); mtf_title = f" MTF {mtf_bias}" if mtf_bias else ""; tf_display = timeframe.upper()
            ...
            try:
                ...
                padding = max((y_high-y_low)*0.08, atr*0.8) if atr else (y_high-y_low)*0.08 if y_high!= y_low else y_high*0.01
                ax.set_ylim(y_low-padding, y_high+padding)
            except: pass
            ax.set_xlim(-1, len(x)+9); ax.grid(True, alpha=0.15, linewidth=0.7)
            ax.spines["top"].set_visible(False); ax.spines["right"].set_visible(False)
            ax.legend(loc="upper left", frameon=False, fontsize=8)
            plt.tight_layout()
            buffer = BytesIO(); buffer.name = "king_zarry_signal.png"
            fig.savefig(buffer, format="png", dpi=140, bbox_inches="tight", facecolor="white"); buffer.seek(0); return buffer
        finally:
            plt.close(fig)
    except Exception as e:
...
def enhance_text_prompt(user_prompt: str) -> str:
    if not groq_client: raise RuntimeError("GROQ_API_KEY is not configured.")
    ...
    if not result: raise RuntimeError("Groq returned empty prompt.")
    return result.strip()
...
def enhance_image_prompt(image_bytes: bytes, motion_prompt: str):
    ...
    result = completion.choices[0].message.content
    if not result: raise RuntimeError("Groq returned empty image prompt.")
    return result.strip()
...
def generate_text_video(prompt: str) -> bytes:
    ...
    if not video_url: raise RuntimeError("Fal.ai returned no video URL.")
    response = requests.get(video_url, timeout=120); response.raise_for_status(); return response.content
...
def save_video(video_bytes: bytes):
    with tempfile.NamedTemporaryFile(delete=False, suffix=".mp4") as temp_file:
        temp_file.write(video_bytes); return temp_file.name
        ...
        for symbol, alerts in alerts_by_symbol.items():
            try:
                ...
                if current_price is None:
                    continue
                now_iso = datetime.now(timezone.utc).isoformat() if 'datetime' in globals() else __import__('datetime').datetime.now(__import__('datetime').timezone.utc).isoformat()
                for alert in alerts:
                    try:
...
async def help_cmd(interaction: discord.Interaction):
    ...
    text = (
        ...
        "`/alert symbol:XAU condition:reaches price:4340` - Create alert\n"
        "`/alerts` - List only your alerts\n"
        "`/cancelalert id:1` - Cancel your alert\n"
        "`/cancelalert id:all` - Cancel all your alerts\n"
        "Natural language: `Alert me when gold reaches 4340`\n\n"
        "**News:**\n"
...
@client.tree.command(name="ping", description="Check King Zarry AI status")
    ...
    try:
        ...
        for sym in symbols:
            try:
                ...
                results.append(f"**{sym}**: {price_str} - {sig}")
            except Exception as e:
...
@client.tree.command(name="adminstatus", description="Admin-only detailed diagnostics")
async def adminstatus(interaction: discord.Interaction):
    if not await require_admin(interaction):
    ...
    try:
        ...
        embed = Embed(title="🛠️ ADMIN DIAGNOSTICS - KING ZARRY AI", description="Private detailed status", color=discord.Color.red())
        embed.add_field(name="🎙️ ElevenLabs", value=f"{eleven_status} | Model: {_redact(ELEVENLABS_MODEL_ID)} | Groq model text: {GROQ_TEXT_MODEL}", inline=False)
        embed.add_field(name="🧠 Groq", value=f"{groq_status} | Vision: {GROQ_VISION_MODEL} | Text: {GROQ_TEXT_MODEL}", inline=False)
        embed.add_field(name="🎬 Fal.ai", value=f"{fal_status} | Text2Video: {TEXT_TO_VIDEO_MODEL} | Image2Video: {IMAGE_TO_VIDEO_MODEL}", inline=False)
        embed.add_field(name="💾 Database", value=f"{db_status}\nMemory: {mem_status}", inline=False)
        embed.add_field(name="📰 News Engine", value=f"Status: {p_status}\nHealth: {health}", inline=False)
        embed.add_field(name="⭐ Premium / Users", value=f"DB: {DATABASE_PATH}", inline=False)
        embed.add_field(name="🔑 Env Check", value=(f"DISCORD_TOKEN: {'FOUND' if DISCORD_BOT_TOKEN else 'MISSING'}\nELEVENLABS_API_KEY: {'FOUND' if ELEVENLABS_API_KEY else 'MISSING'}\nGROQ_API_KEY: {'FOUND' if GROQ_API_KEY else 'MISSING'}\nFAL_KEY: {'FOUND' if FAL_KEY else 'MISSING'}\nTWELVE_DATA_API_KEY: {'FOUND' if os.getenv('TWELVE_DATA_API_KEY') else 'MISSING'}"), inline=False)
        await interaction.response.send_message(embed=embed, ephemeral=True)
    except Exception as e:
        logger.error("adminstatus error: " + _redact(repr(e)))
        await interaction.response.send_message(f"❌ Admin diagnostic error: {_redact(str(e))[:500]}", ephemeral=True)
