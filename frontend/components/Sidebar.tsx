        ...
        <div className="flex items-center justify-between px-4 py-3">
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-[9px] font-mono text-cyan-400/60 tracking-widest">
                COMMAND CENTRE
              </p>
            </div>
          </div>
          <button
            onClick={logout}
            className="px-3 py-1.5 rounded-md text-[10px] font-mono tracking-widest text-red-400/70 border border-red-500/20 hover:bg-red-500/10"
          >
            EXIT
          </button>
        </div>
        <nav className="flex gap-1 overflow-x-auto kz-scroll px-2 pb-2">
          {navItems.map((item) => (
            <Link
              key={item.href}
              href={item.href}
              className={`flex-shrink-0 px-3 py-2 rounded-md text-[10px] font-mono tracking-wider transition-all ${
                isActive(item.href)
        ...
        <div className="p-5 border-b border-cyan-500/10">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[10px] font-mono text-cyan-400/60 tracking-widest">
                COMMAND CENTRE
              </p>
            </div>
          </div>
        ...
        <nav className="flex-1 p-3 space-y-1">
          {navItems.map((item) => (
            <Link
              key={item.href}
              href={item.href}
              className={`flex items-center gap-3 px-3 py-2.5 rounded-lg text-xs font-mono tracking-wider transition-all duration-200 ${
                isActive(item.href)
        ...
        <div className="p-4 border-t border-cyan-500/10">
          {user && (
            <div className="mb-3">
              ...
              <p className="text-[10px] font-mono text-cyan-400/40 truncate">
                {user.email}
              </p>
            </div>
          )}
          <button
            onClick={logout}
            className="w-full px-3 py-2 rounded-lg text-[10px] font-mono tracking-widest text-red-400/70 border border-red-500/20 hover:bg-red-500/10 hover:text-red-300 transition-all"
          >
            SIGN OUT
          </button>
        </div>
