...
function getOHLC(
  c: ChartCandle,
): { o: number; h: number; l: number; cl: number } | null {
...
export default function TradingChart(props: TradingChartProps) {
  ...
  const currentPrice = safeStr(
    props.currentPrice ?? props.price ?? props.lastPrice ?? null,
  );

  const change = safeStr(props.change ?? null);

  const changePct = safeStr(
    props.changePercent ?? props.percentChange ?? null,
  );
  ...
  const directionRaw =
    props.direction ?? props.side ?? props.bias ?? null;

  const dirInfo = normalizeDir(
    directionRaw ? String(directionRaw) : null,
  );
  ...
  const stats = useMemo(() => {
    ...
    [entry, sl, tp1, tp2, tp3].forEach((v) => {
  ...
  if (loading) {
    return (
      <section
        className={`kz-panel relative overflow-hidden rounded-[24px] border border-white/[0.07] bg-[#0A0E16] ${
          props.className || ""
        }`}
        aria-busy="true"
      >
        <div
          aria-hidden="true"
          className="absolute inset-0 bg-[linear-gradient(to_right,#ffffff06_1px,transparent_1px),linear-gradient(to_bottom,#ffffff06_1px,transparent_1px)] bg-[size:32px_32px] opacity-30"
        />

        <div className="relative">
          <div className="flex h-[56px] items-center justify-between border-b border-white/[0.06] px-6">
          ...
          <div className="p-6">
            <div className="relative h-[320px] overflow-hidden rounded-[16px] border border-white/[0.06] bg-gradient-to-b from-white/[0.04] to-white/[0.01]">
  ...
  if (error) {
    return (
      <section
        className={`relative rounded-[24px] border border-rose-400/20 bg-[#0E0A0D] p-6 ${
          props.className || ""
        }`}
        role="alert"
      >
        <div className="text-[11px] tracking-[0.14em] text-rose-300/60">
  ...
  if (!normalized.length || !stats) {
    return (
      <section
        className={`relative rounded-[24px] border border-white/[0.06] bg-[#0A0E16] p-6 ${
          props.className || ""
        }`}
      >
        <div className="text-[10px] tracking-[0.14em] text-white/30">
          MARKET DATA UNAVAILABLE
        </div>

        <div className="mt-2 text-[13px] text-white/40">
          No chart data. Awaiting real data from props. No fabricated
          candles.
        </div>
      </section>
    );
  ...
  const xStep =
    chartW / Math.max(1, normalized.length - 1);
  ...
  const priceLinePoints = normalized
    .map((c, i) => {
      const p = getCandlePrice(c);

      if (p === null) return null;

      return `${padL + i * xStep},${yScale(p)}`;
    })
    .filter(Boolean)
    .join(" ");

  const ema9Points = normalized
    .map((c, i) => {
      const v = safeNum(c.ema9);

      if (v === null) return null;

      return `${padL + i * xStep},${yScale(v)}`;
    })
    .filter(Boolean)
    .join(" ");

  const ema21Points = normalized
    .map((c, i) => {
      const v = safeNum(c.ema21);
  ...
  const ema50Points = normalized
    .map((c, i) => {
      const v = safeNum(c.ema50);
  ...
  const hoverCandle =
    hoverIdx !== null ? normalized[hoverIdx] : null;

  const hoverPrice =
    hoverCandle ? getCandlePrice(hoverCandle) : null;

  return (
    <section
      className={`kz-panel kz-glass relative overflow-hidden rounded-[24px] border border-white/[0.08] bg-gradient-to-b from-[#0F1728] to-[#080B14] shadow-[0_20px_64px_rgba(0,0,0,0.6),inset_0_1px_0_rgba(255,255,255,0.07)] md:rounded-[28px] ${
        props.className || ""
      }`}
      aria-label={`Trading chart ${symbol ?? ""} ${
        timeframe ?? ""
      }`.trim()}
    >
      <div
      ...
      <span
        aria-hidden="true"
        className="pointer-events-none absolute right-3 top-3 h-5 w-5 rounded-tr-[20px] border-r border-t border-cyan-300/20"
      />

      <div className="relative">
        {!props.hideHeader && (
          <div className="flex h-auto flex-col justify-between gap-3 border-b border-white/[0.06] px-6 py-4 md:h-[64px] md:flex-row md:items-center md:py-0">
            <div className="flex min-w-0 items-center gap-4">
              <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-[10px] border border-white/10 bg-white/[0.06]">
                <span
                  className="h-2 w-2 animate-pulse rounded-full bg-cyan-300 shadow-[0_0_10px_rgba(34,211,238,0.7)]"
                  aria-hidden="true"
                />
              </div>
              ...
              <div className="min-w-0">
                <div className="flex flex-wrap items-center gap-2.5">
                  ...
                  {timeframe && (
                    <span className="rounded-full border border-white/10 bg-white/[0.04] px-2.5 py-1 text-[10px] tracking-[0.08em] text-white/45">
                      {timeframe}
                    </span>
                  )}
                  ...
                  {dirInfo && (
                    <span
                      className={`rounded-full border px-2.5 py-1 text-[10px] font-medium tracking-[0.08em] ${
                        dirInfo.isLong
                          ? "border-cyan-300/20 bg-cyan-400/[0.08] text-cyan-200/80"
                          : dirInfo.isShort
                            ? "border-rose-300/20 bg-rose-400/[0.08] text-rose-200/75"
        ...
        <div className="p-3 md:p-5">
          <div className="overflow-hidden rounded-[18px] border border-white/[0.06] bg-[#0A0F1B]/80 backdrop-blur">
            <div className="relative">
              <svg
                viewBox={`0 0 ${W} ${H}`}
                className="block h-[360px] w-full select-none md:h-[400px]"
                role="img"
                aria-label={`Price chart for ${
                  symbol ?? "market"
                }`}
              >
                {!props.hideGrid && (
                ...
                {hasOHLC ? (
                  <g>
                    {normalized.map((c, i) => {
                      ...
                      const yO = yScale(ohlc.o);
                      const yC = yScale(ohlc.cl);
                      const yH = yScale(ohlc.h);
                      const yL = yScale(ohlc.l);

                      const isBull = ohlc.cl >= ohlc.o;
              ...
              {hoverCandle && hoverIdx !== null && (
                <div className="pointer-events-none absolute left-4 top-4 rounded-[12px] border border-white/[0.08] bg-[#0E1524]/90 px-3 py-2.5 backdrop-blur">
          ...
          {aiText && (
            <div className="mt-4 rounded-[14px] border border-cyan-300/12 bg-gradient-to-b from-cyan-400/[0.06] to-white/[0.01] p-[1px]">
              <div className="rounded-[13px] bg-[#0A131D]/90 p-4 backdrop-blur">
                <div className="text-[10px] tracking-[0.12em] text-cyan-200/60">
                  AI MARKET INTELLIGENCE
                </div>

                <p className="mt-2.5 whitespace-pre-wrap text-[12.5px] leading-[1.6] text-white/70">
                  {aiText}
                </p>
              </div>
            </div>
