...
export type UpdateAlertPayload = Partial<
  Pick<Alert, "symbol" | "target_price" | "condition" | "active">
>;
...
interface AlertsApi {
  ...
  deleteAlert?: (id: Alert["id"]) => Promise<void>;
}
...
export function useAlerts(
  ...
  const apiRef = useRef<AlertsApi | null>(null);
  ...
  const getApi = useCallback(async (): Promise<AlertsApi | null> => {
    if (apiRef.current) {
      return apiRef.current;
    }

    const api = await loadAlertsApi();
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      if (api.getAlerts) {
        fetched = await api.getAlerts();
      } else if (api.listAlerts) {
        fetched = await api.listAlerts();
      } else if (api.fetchAlerts) {
        fetched = await api.fetchAlerts();
      }
      ...
      setError(
        getErrorMessage(error, "Failed to load alerts")
      );
    } finally {
      if (
        requestId === requestIdRef.current &&
  ...
  >(
    async (payload) => {
      ...
      try {
        ...
        if (!api?.createAlert) {
          const message =
            "Alert creation API is not connected yet";
        ...
        const id = extractAlertId(result);
  ...
  >(
    async (id, payload) => {
      ...
      try {
        ...
        if (!api?.updateAlert) {
          const message =
            "Alert update API is not connected yet";
  ...
  >(
    async (id) => {
      ...
      try {
        ...
        if (!api?.deleteAlert) {
          const message =
            "Alert deletion API is not connected yet";
