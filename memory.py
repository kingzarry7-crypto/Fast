...
class Memory:
    ...
    def _get_connection(self):
        # timeout=30 prevents 'database is locked' errors under concurrent usage
        ...
        with self.lock:
            with self._get_connection() as conn:
                ...
                cursor.execute("""
                    ...
                    ON conversation_memory(user_id, timestamp)
                """)
                conn.commit()
    ...
    def get_history(self, user_id: str, limit: int = 10) -> list:
        with self.lock:
            with self._get_connection() as conn:
                ...
                # Fetch recent items within retention period in chronological order
                cutoff = datetime.now(timezone.utc) - timedelta(hours=self.retention_hours)
                cutoff_str = cutoff.strftime("%Y-%m-%d %H:%M:%S")
...
# Vision Models
GROK_VISION_MODEL = clean_env_str(os.environ.get("GROK_VISION_MODEL") or os.environ.get("GROQ_VISION_MODEL"), "llama-3.2-11b-vision-preview")
        ...
        try:
            ...
            if message.attachments:
                ...
                if any(attachment.filename.lower().endswith(ext) for ext in [".png", ".jpg", ".jpeg", ".webp"]):
                    ...
                    # Fetch history first BEFORE appending current prompt
                    history = await asyncio.to_thread(memory.get_history, user_id, 10)
                    
                    # Store current user prompt
                    memory_prompt = f"[User attached an image] {prompt}".strip()
                    await asyncio.to_thread(memory.add, user_id, "user", memory_prompt)

                    response = await asyncio.to_thread(analyze_image_with_ai, image_bytes, mime_type, prompt)
                    
                    # Store assistant response
                    await asyncio.to_thread(memory.add, user_id, "assistant", response)
            ...
            # 1. Fetch history BEFORE adding current prompt
            history = await asyncio.to_thread(memory.get_history, user_id, 10)

            # 2. Add current user text prompt
            await asyncio.to_thread(memory.add, user_id, "user", clean_text)

            # 3. Get response passing historical context
...
async def clear_memory(interaction: discord.Interaction):
    ...
    await asyncio.to_thread(memory.clear, user_id)
